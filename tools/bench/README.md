...
`--comparators-only` runs the independent FAISS/hnswlib lanes selected by `--algorithms` without constructing an ovVS index. This is useful for repeated comparator A/B controls when accelerator contention would waste or invalidate an ovVS lane. The artifact records the mode, is explicitly noncanonical, and cannot establish an ovVS speedup or close B1. It cannot be combined with `--include-hnsw-export`, because that hybrid lane has an ovVS producer.
