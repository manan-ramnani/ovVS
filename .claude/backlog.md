...
The C ABI covers brute-force, IVF-Flat/PQ/RaBitQ, CAGRA, NN-Descent, Vamana, ScaNN, HNSW-from-CAGRA, all-neighbors, filters, dynamic batcher, k-means, SLINK, spectral, PQ/SQ/binary, PCA, pairwise, k-selection. Python ctypes + NumPy/DLPack ingest exist. Most algorithm tests remain toy-sized (**n=24–80**); bounded NN-Descent GPU regressions cover 4,097 and 16,384 rows. Three clean current-code SIFT1M CAGRA gates pass the plan's recall-closeness condition at the low-effort point: 0.9036 recall@10 versus hnswlib median 0.8912. The same graph reaches the separate 0.95 target at CAGRA `itopk=64/search_width=2` (0.9609). Exact cached-worst tracking, prefix-only publication sorting, canonical 64-ID frontier tiling, and subgroup min-location selection for multi-pick search are promoted. The cooperative-pick checkpoint preserved bitwise IDs/distances and improved median QPS by 17.8–29.9% across the three multi-pick rows in three serial-baseline and three candidate SIFT1M processes; the width-one serial-selection semantics showed non-attributable +2.0% variation, treated as neutral. Search performance is still negative: at the nearest ≥0.95 point current CAGRA reaches 3,243.5 QPS versus hnswlib 6,784.5, a 2.09× deficit; near 0.989 recall the deficit is 3.13×, and the higher-recall hnswlib batch-one point remains 42.55× faster. The proposed four-workgroup owner/helper batch-one route is parked on Arrow Lake because its exact named-kernel gate reported one cooperative workgroup versus four required and submitted no kernel. The exported single-layer graph loads, searches, saves, and reloads through stock hnswlib, but three clean SIFT100K diagnostics found it slower to build and search than native hnswlib. Exact T13.3 GPU detour-rank prune/reverse merge is promoted through intermediate degree 64. The later profiler-guided cooperative-minima join retained exact 100K/1M graph bytes and all lifecycle counters while reducing three-process SIFT1M median complete build from 91.978 to 61.000 seconds (-33.68%) and initializer wall from 90.883 to 60.164 seconds (-33.80%); recall remained 0.9036, search QPS/p99 changed +0.04%/+0.03%, and peak RSS changed +0.14%. Full FORCE_GPU NN-Descent CAGRA construction remains available for supported shapes. In the same candidate cohort ovVS built 20.87% faster than the unchanged packaged hnswlib comparator, but the installed Windows hnswlib 0.8.0 binary uses legacy SSE distance code. The separate source-pinned MSVC `/arch:AVX2` comparator, exact module/provenance loader, and batch-cap 256/1024 lanes now exist; its hash-matched import smoke passes and static binary has AVX float operations absent from stock. Repeated complete SIFT1M measurements are still required before any full-speed hnswlib claim. Four synchronization/readback candidates remain rejected and production-restored, and bank-safe T64 plus NEW-only coordinate staging were rejected after 2.51×/2.23× SIFT100K regressions.

The AVX2 comparator claim is limited to reproducible build and static/import validation. Two fresh roots produced the same wheel and module; the harness now rejects incomplete full-profile point sets and semantically invalid provenance. No active-cycle or SIFT1M performance result exists yet. Evidence: `tables/arrow-lake/evidence/hnswlib-avx2-build-v1/README.md`.
...
| ID | Item | Plan | Notes |
|---|---|---|---|
| B1 | SIFT1M / 1e5×768 recall–QPS harness vs FAISS-CPU and hnswlib; AUTO + FORCE_*; µJ/query | T22.1–T22.2, §8–§9 | **PARTIAL:** strict CLI/JSON harness, bounded smoke validation, independent construction/search policy, fail-closed matched SIFT1M and prefix gates, loaded-DLL fingerprints, and an opt-in CAGRA→stock-hnswlib lane are complete. The current cooperative-pick checkpoint adds a clean repeated/interleaved CAGRA A/B curve: three serial-baseline and three candidate processes, complete tails and package energy, exact truth, and unchanged hnswlib settings. Current CAGRA reaches 0.9609 recall and 3,243.5 QPS versus the packaged hnswlib at 0.9593 and 6,784.5 QPS. That Windows extension's float-distance arithmetic uses legacy SSE, and its binding forces one search thread for batches 1 and 32. A separately source-pinned hnswlib 0.8.0 MSVC `/arch:AVX2` builder now emits a semantically validated, hash-matched module/provenance pair; the harness records its exact binary and inferred binding-effective threads, and SIFT1M has targeted comparable-recall batch-cap 256/1024 points without changing M/ef settings. Reproducible import/static-SIMD validation passes; repeated end-to-end AVX2 runs remain open. Other algorithms/policies, matched FAISS comparisons, a same-OS control before any WSL ScaNN product comparison, and a real 100K×768 corpus report remain open. Evidence: `tables/arrow-lake/cagra-build-v1.md`, `tables/arrow-lake/cagra-search-v1.md`, `tables/arrow-lake/cagra-cooperative-pick-v1.md`, `tables/arrow-lake/hnsw-export.md`, `tables/arrow-lake/evidence/hnswlib-avx2-build-v1/README.md`. |
| B2 | CAGRA search kernel T13.4: one **work-group** per query, SLM itopk, bounded hashmap (not `Seen[nq×n]`), graph-aware seeds, subgroup distance | T13.4, then T13.5–T13.6 | **PARTIAL / COOPERATIVE PICK PROMOTED; PERFORMANCE OPEN:** work-group/query, cooperative distance, SLM candidates, and a bounded visited hash are implemented. Exact cached-worst tracking first delivered 81.9–156.5% QPS gains; prefix-only sorting and canonical frontier tiling added 8.0–10.9%. Exact subgroup min-location selection for `search_width > 1` then passed a separate six-process gate: multi-pick QPS improved 17.8%, 24.6%, and 29.9%, p50 fell 15.2–22.9%, and p99 fell 15.4–20.8%; the width-one serial-selection semantics showed non-attributable +2.0% variation, treated as neutral. Recall stayed unchanged, the cross-DLL fixture matched ID and distance bytes, build wall changed -0.33%, and peak RSS was unchanged. Product performance still loses: hnswlib is 2.09× faster at the nearest ≥0.95 point, 3.13× near 0.989 recall, and 42.55× at the non-recall-matched batch-one point. A stable one-pass pick was exact but rejected after regressing three of four effort points. The proposed four-workgroup owner/helper route is also parked on Arrow Lake: the exact 128-thread/2,640-byte named-kernel gate reported `max_num_work_groups=1`, exited 77, and launched nothing. Continue B2 only through software designs that do not require unsupported cross-workgroup synchronization, while preserving the promoted one-workgroup path. Evidence: `tables/arrow-lake/cagra-cooperative-pick-v1.md` and `tables/arrow-lake/gpu-root-group-canary-v1.md`. |
...
| B5 | NN-Descent iGPU local-join + bloom for n≫4096 | T12 | **PARTIAL / GPU OPTIMIZER AND COOPERATIVE MINIMA PROMOTED; T12.5 OPEN:** n≤4096 remains exact kNN. Larger L2/L2-sqrt/IP/cosine builds use deterministic cached-distance NEW/OLD tagging, bounded forward/reverse samples, reciprocal proposals, target-owned merges, and exact changed/pending-NEW accounting on SYCL. Exact T13.3 optimize/prune/merge runs on iGPU through intermediate degree 64. The profiler-guided join now computes up to 96 row/column minima cooperatively, then preserves lane-0 emission order. Three AUTO SIFT1M builds completed in 60.909–62.604 s (61.000 s median), with a 60.164 s initializer median, 0.655 s optimizer median, 2,272.9 MiB median peak RSS, exact 100K/1M graph bytes, unchanged 0.9036 recall, and identical lifecycle counters. This is -33.68% complete wall versus the current product parent and 20.87% faster to build than the packaged stock hnswlib cohort, subject to the explicit SSE-comparator caveat in B1. Full FORCE_GPU build remains available for supported shapes. All runs still report 1,098 kernels, 2,458 submissions, 1,820 waits, 459 D2H calls, zero H2D, and six iterations without convergence. Four synchronization/readback candidates and two coordinate-staging layouts remain reverted negatives. Reprofile the promoted kernel before the next one-variable change; separately measure a device-resident initializer-to-optimizer handoff. T12.5 IVF-PQ-initializer comparison and K64 scale remain open. Evidence: `tables/arrow-lake/nndescent-cooperative-minima-v1.md`, `tables/arrow-lake/cagra-gpu-optimize-v1.md`, and the rejection reports. |
...
| B9 | CAGRA knobs: `min/max_iterations`, `team_size`, hashmap mode, `filtering_rate` → itopk; kernel portfolio and dynamic batching | T13.5–T13.8 | Search ABI today exposes `itopk_size` and `search_width` only. Build a measured SIMD16 portfolio around the admitted one-workgroup path and bounded multi-kernel/persistent fallbacks. The four-workgroup cooperative small-batch route and Arrow Lake NPU candidate-slab offload are parked; reopen either only after a materially different complete capability/performance result. |
...
1. Do not implement the proposed four-workgroup **B2** owner/helper route on the measured Arrow Lake stack: its fail-closed named-kernel gate reports one cooperative workgroup versus four required. Preserve the promoted one-workgroup kernel. Admit another B2 search change only from profiling evidence, with a software-only design that avoids unsupported cross-workgroup synchronization and passes exact old/new outputs plus complete p50/p99/QPS/recall evidence against unchanged hnswlib. Otherwise proceed to the measured B5 construction bottleneck. The exact stable one-pass pick is also rejected evidence and must not be revived without a materially different design.
2. Do not continue unprofiled **GPU NN-Descent initializer** synchronization changes: fixed-`N` active counts (+8.91% complete wall), convergence reduction (+2.14%), target-owned head reset (+5.55%), and producer→copy event chaining (+8.96%) all lost. Preserve the production path. Reopen initializer work only from device-side phase evidence with a design that reduces actual algorithmic work while preserving exact graph quality.
3. T13.3 iGPU optimize/prune/merge is promoted through intermediate degree 64. The remaining construction target is the 90.883-second GPU initializer: profile actual device phases, then separately measure removal of the final initializer D2H plus optimizer H2D handoff. Matching current-cohort hnswlib requires another 14.17 seconds, about 15.6% of initializer wall. The measured export-only route remains parked.
...
This is one benchmark invocation with two sequential isolated lane processes and no in-run clock, thermal, utilization, or background-load trace, not a promotion claim. It moved the primary CAGRA search target first to exact cached-worst candidate tracking and then deterministic SIMD16 cooperative selection; both later passed separate promotion gates. The subsequent four-workgroup/query small-batch capacity gate failed closed and parked that route on the measured Arrow Lake stack. Construction synchronization/readback remains a separate measured build target. Details: `tables/arrow-lake/cagra-search-v1.md`.
...
### 2026-08-29 — CAGRA four-workgroup capacity gate

The default-off `ovvs_gpu_root_group_canary` tests the necessary admission
condition for the proposed batch-one owner/helper route without linking or
routing `libovvs`. At clean commit `6deb173`, the named executable kernel used
128 work-items and the conservative current `128/4` CAGRA local-memory
footprint of 2,640 bytes per group. oneAPI Unified Runtime over Level Zero
reported `max_num_work_groups=1` versus four required. The producer exited 77,
submitted no root-barrier kernel, used no fallback, and made no acceleration
claim. The registered CTest has an Intel-GPU resource lock and 20-second bound.

The four-workgroup owner/helper design is therefore parked on this Arrow Lake
software stack. Reopen only for a materially changed runtime/driver, another
SKU, or a design with a different measured cooperative requirement; no BIOS or
firmware change is requested. Immutable evidence and executable provenance:
`tables/arrow-lake/gpu-root-group-canary-v1.md`.

### 2026-08-29 — NN-Descent active-count experiment rejected

A bounded candidate kept reverse/proposal active counts device-resident and
replaced host-sized dynamic consumer launches with fixed-`N` consumers linked
by explicit SYCL events. Eight focused GPU/graph tests passed after fail-closed
producer bounds guards. At SIFT100K the valid parent/candidate pair was neutral,
with unchanged 0.9718 recall and -0.19% complete build variation.

The fresh SIFT1M pair was decisive. D2H calls fell 459→21, submissions
2,458→2,020, and literal waits 1,820→488, while kernels, iterations, final
changed/pending counts, recall 0.9036, allocation accounting, and zero H2D were
unchanged. However, complete build regressed 100.335→109.275 s (+8.91%) and
initializer wall 90.906→99.864 s (+9.85%); hnswlib build drift was only -0.68%.
The patch was rejected and production source restored. Two earlier SIFT100K
attempts with incomplete DLL dependency closures remain retained as invalid
unavailable lanes. This was one admission pair, not a repeated promotion study,
and no cross-DLL raw graph-byte identity is claimed. Next preserve dynamic
consumer ranges and reduce only convergence arrays on-device. Evidence:
`tables/arrow-lake/nndescent-active-count-v1.md`.

### 2026-08-29 — NN-Descent convergence reduction rejected

The second bounded B5 candidate preserved all reverse/proposal scalar
readbacks and dynamic consumer ranges. It replaced only the two full
convergence arrays per iteration with one dependent 128-work-item reduction,
a validated 24-byte summary, and one event wait. The typed control block added
24 device bytes and changed no public or serialized ABI. It built with oneAPI
2025.1.1 and passed eight focused GPU/graph tests with zero skips.

At SIFT1M, D2H bytes fell 176,003,272→128,003,416 (-27.27%), calls 459→453,
and waits 1,820→1,814; submissions stayed 2,458 and kernels rose 1,098→1,104.
Recall 0.9036, six iterations, final 5,911,309 changed / 5,949,466 pending,
allocation count, and zero H2D were preserved. The complete build nevertheless
regressed 99.863→102.004 s (+2.14%) and initializer wall regressed
90.515→92.240 s (+1.91%); hnswlib build drift was +0.72%. SIFT100K initializer
wall was neutral at +0.04%.

The candidate is rejected, production source is restored, and no graph-byte or
search-performance claim is made. Each size has one candidate/parent pair with
energy disabled, sufficient for rejection but not promotion-shaped evidence.
Next remove repeated scratch initialization: fill `heads[N]` once per iteration
and have each unique target-owned consumer reset only the rows it touched.
Evidence: `tables/arrow-lake/nndescent-convergence-reduction-v1.md`.

### 2026-08-29 — NN-Descent target-owned head reset rejected

The third bounded B5 candidate initialized `heads[N]` once per iteration and
had each unique target-owned reverse/proposal consumer clear the entry it had
traversed. Active-count readbacks, dynamic consumer ranges, convergence, and
host termination were unchanged. Independent ownership/race audits passed, the
candidate built with oneAPI 2025.1.1, and eight focused GPU/graph tests passed
with zero skips. The bounded scale fixture temporarily crossed the proposal
chunk seam at 16,385 rows.

At SIFT1M the exact structural target was reached: submissions fell
2,458→2,020 (-17.82%), while 1,098 kernels, 1,820 waits, 459 D2H calls /
176,003,272 bytes, allocations, zero H2D, six iterations, final
5,911,309 changed / 5,949,466 pending edges, and recall 0.9036 were unchanged.
The complete build nevertheless regressed 99.906→105.449 s (+5.55%) and the
initializer regressed 90.502→96.082 s (+6.17%). hnswlib build moved -0.81% in
the earlier candidate process, so the result is not attributed to general
machine slowdown. SIFT100K initializer wall was neutral at +0.05%, again
showing that the prefix is admission rather than a scale predictor.

The candidate is rejected and production source/tests are restored. One
excluded first attempt with an incomplete Windows DLL dependency closure is
retained explicitly and contributes no metric. One valid pair per size, no
energy, and no cross-DLL graph digest are sufficient for rejection but not
promotion. Next keep the bulk fills and dynamic consumers and remove only the
redundant producer queue barrier through an explicit dependency into the
active-count copy. Evidence:
`tables/arrow-lake/nndescent-heads-reset-v1.md`.

### 2026-08-29 — NN-Descent producer-to-copy event chain rejected

The fourth bounded B5 candidate retained every bulk head/scalar fill, active
count copy, dynamic consumer range, consumer wait, and convergence readback. It
captured reverse/proposal producer events and made each scalar D2H copy depend
on its producer, replacing only the redundant queue-wide producer wait. The
final implementation used the standard direct `queue::memcpy(..., depEvent)`
overload. Eight focused GPU/graph tests passed with zero skips, including a
temporary 16,385-row proposal-chunk seam fixture.

At SIFT1M waits fell exactly 1,820→1,376 (-24.40%). Submissions 2,458,
kernels 1,098, D2H 459 / 176,003,272 bytes, allocations, zero H2D, six
iterations, final 5,911,309 changed / 5,949,466 pending edges, and recall 0.9036
were unchanged. Complete build nevertheless regressed 100.098→109.062 s
(+8.96%) and initializer wall regressed 90.602→99.367 s (+9.68%). The earlier
candidate process's hnswlib build was 3.32% faster, arguing against a global
process slowdown; one non-interleaved pair cannot exclude phase-specific
machine noise.

A handler-submitted copy variant first regressed SIFT100K initializer wall
6.42%; the direct overload made that prefix initializer neutral at +0.04%, but
did not survive the full-scale screen. Both diagnostics and the direct source
patch are retained. Production source/tests are restored. One pair per size,
no energy, and no cross-DLL graph digest are sufficient for rejection but not
promotion. Four synchronization/counter-reduction designs have now lost;
further initializer work requires device phase profiling and less algorithmic
work. T13.3 iGPU optimize/prune/merge is next. Evidence:
`tables/arrow-lake/nndescent-producer-copy-event-v1.md`.
...
The clean build-stage evidence first isolated GPU NN-Descent and host optimize/prune/merge. The exact T13.3 GPU optimizer subsequently removed 92.90% of the latter stage and reduced repeated complete SIFT1M build wall 8.10%; NN-Descent now dominates the remaining construction wall. The later same-index curve moved the primary search target to cached-worst tracking, which subsequently passed its repeated complete-wall gate. Prefix-only publication sorting, canonical frontier tiling, and cooperative multi-pick selection then passed separate repeated gates; the exact stable one-pass pick was rejected after a negative admission screen. The subsequent owner/helper capacity gate reported one cooperative workgroup versus four required and parked that route on the measured Arrow Lake stack. Hierarchical/packed iGPU PQ and size-gated CPU+iGPU batching remain on the critical path. No BIOS, firmware, driver, or power setting changed.
...
### 2026-08-29 — T13.3 exact GPU optimizer promotion

Commit `28f2dbd` ports the existing detour-rank reorder, fixed-degree prune,
capped reverse construction, and deterministic forward/reverse interleave to
SYCL through intermediate degree 64. Adaptive policies use the GPU when the
shape and device are supported and fall back to the CPU only for that bounded
cap or device absence; selected GPU runtime/correctness failures remain
fail-closed. Full FORCE_GPU NN-Descent CAGRA construction now completes for
supported degrees. The public C ABI, frozen telemetry layout, and serialized
index format are unchanged. Verification was 95/95 native tests with zero
skips and eight passed CTest product lanes; the separate unsupported
root-group capability canary skipped as designed.

Three AUTO SIFT1M processes reduced median optimizer wall from 9.034 to 0.641
seconds (-92.90%) and complete build from 100.093 to 91.987 seconds (-8.10%).
Recall remained 0.9036, initializer lifecycle counters were identical, and
peak RSS fell 3.13%. Separate parent/candidate processes produced identical
dataset-free graph bytes at both SIFT100K and SIFT1M; the SIFT1M graph payload
SHA-256 is `59CADBE6…B315`. The earlier generic 10% whole-build gate is retained
for initializer/multi-stage changes, but cannot be met by an isolated stage
that occupied only 9.03% of baseline wall. This scoped promotion captures
about 89.7% of that ceiling without a correctness or memory regression.

At this checkpoint energy was disabled, the SIFT1M artifacts were quality
gates rather than full B1 curves, and ovVS still built 1.182× slower than
unchanged hnswlib. The later cooperative-minima entry supersedes the
construction timing and comparator disposition after profiler-guided
initializer work. Evidence: `tables/arrow-lake/cagra-gpu-optimize-v1.md`.

### 2026-08-29 — NN-Descent cooperative-minima promotion

VTune attributed 82.113 of 91.034 SIFT1M GPU seconds to the proposal join,
with 22.8% occupancy, 10.8% XVE active time, 86.1% stalled time, and no material
L3-bandwidth limit. The promoted change assigns the already-computed
`2*new_count+old_count <= 96` row/column minima to work-items. Each scan keeps
the prior ascending order and `(distance, ID)` comparison; a uniform barrier
then lets lane 0 retain the original reciprocal-emission and atomic order.
Local memory rises 8,456→9,224 bytes. Public ABI, serialization, distance
association, submissions, waits, transfers, and telemetry counters are
unchanged.

Three parent and three candidate SIFT100K processes reduced median complete
build 7.913→5.346 seconds (-32.44%). Three current-product parent and three
candidate SIFT1M processes reduced it 91.978→61.000 seconds (-33.68%) and the
initializer 90.883→60.164 seconds (-33.80%). Recall stayed 0.9036; graph bytes,
six-iteration lifecycle counters, and the canonical 1M SHA-256 were exact.
Median search QPS/p99 changed +0.04%/+0.03% and peak RSS +0.14%. Verification
was 95/95 native tests and eight passed CTest product lanes; the unsupported
root-group canary skipped as designed.

The same cohort put ovVS construction 20.87% ahead of the unchanged packaged
hnswlib build, but this is not yet a full-speed competitor claim: the installed
hnswlib 0.8.0 Windows binary uses SSE distance code. A pinned `/arch:AVX2`
comparator and batch 256/1024 throughput lanes are open under B1. Stock
hnswlib still wins low-effort search, and the existing comparable-recall search
deficit remains.

Bank-safe 64-dimension NEW+OLD tiling and full-dimension NEW-only staging were
rejected after single SIFT100K screens ran 2.51× and 2.23× slower with unchanged
recall and counters. No BIOS, firmware, driver, or power setting changed.
Evidence: `tables/arrow-lake/nndescent-cooperative-minima-v1.md`.
...
- At assessment time NN-Descent / CAGRA init did not scale. The current bounded SYCL join now completes the full SIFT1M AUTO build and passes the CAGRA-vs-hnswlib recall-closeness gate; T13.3 GPU prune is now promoted, while B5 remains partial for T12.5 convergence/IVF-PQ-init comparison and K64 scale.
- HETERO not wired (B6).
