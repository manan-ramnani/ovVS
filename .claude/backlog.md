...
| B5 | NN-Descent iGPU local-join + bloom for n≫4096 | T12 | **PARTIAL / FOUR SYNC DESIGNS REJECTED; T12.5 OPEN:** n≤4096 remains exact kNN. Larger L2/L2-sqrt/IP/cosine builds use deterministic cached-distance NEW/OLD tagging, bounded forward/reverse samples, reciprocal proposals, target-owned merges, and exact changed/pending-NEW accounting on SYCL. Three clean SIFT1M AUTO constructions completed in 99.874–100.525 s (100.093 s median) within 2,343.2 MiB median isolated peak RSS and supported 0.9036 downstream CAGRA recall. The GPU initializer contributes 90.444 s (90.36%) of median complete wall; host optimize/prune/merge contributes 9.034 s. Each build reports 1,098 kernels, 2,458 submissions, 1,820 waits, 459 D2H calls, zero H2D, and six iterations without convergence. Fixed-`N` active counts (+8.91% complete wall), convergence reduction (+2.14%), target-owned head reset (+5.55%), and producer→copy event chaining (+8.96%) were all reverted after exact structural targets but negative complete walls. Further initializer changes require device-side phase evidence and less algorithmic work. The next implementation target is T13.3 iGPU optimize/prune/merge. T12.5 IVF-PQ-initializer comparison and K64 scale remain open. Evidence: `tables/arrow-lake/cagra-build-v1.md`, `tables/arrow-lake/nndescent-active-count-v1.md`, `tables/arrow-lake/nndescent-convergence-reduction-v1.md`, `tables/arrow-lake/nndescent-heads-reset-v1.md`, `tables/arrow-lake/nndescent-producer-copy-event-v1.md`. |
...
3. Move the measured 9.034-second host graph optimize/prune/merge stage to the iGPU as the next separately attributable construction change. Keep the CPU path until exact graph validation and repeated complete wall prove a winner. The measured export-only route is parked.
