...
| B5 | NN-Descent iGPU local-join + bloom for n≫4096 | T12 | **PARTIAL / CAGRA-VS-HNSWLIB GATE PASSED; T12.5 OPEN:** n≤4096 remains exact kNN. Larger L2/L2-sqrt/IP/cosine builds use deterministic cached-distance NEW/OLD tagging, bounded forward/reverse samples, reciprocal proposals, target-owned merges, and exact changed/pending-NEW convergence accounting on SYCL. Current-code SIFT1M AUTO construction completed in 185.3 s within 2,342.5 MiB isolated peak RSS and supported 0.9036 downstream CAGRA recall. The final optimizer primitive remains CPU, and the current harness reports only the last primitive rather than initializer/iteration/optimizer/copy attribution. First add versioned, success-only build-stage telemetry; then move measured reduction, optimize/prune, and merge bottlenecks to the iGPU. Standalone convergence, T12.5 IVF-PQ-initializer comparison, K64 scale, and T13.3 remain open. |
...
1. Add explicit, versioned, success-only **CAGRA build-stage telemetry** so initializer, NN-Descent iterations, optimizer/prune/merge, copies, allocations, submissions, waits, and peak memory are measured rather than inferred.
2. Use that evidence to move CAGRA/NN-Descent construction and graph optimize/prune/merge work to the iGPU where complete build plus unchanged stock-hnswlib CPU search improves. The measured export-only route is parked.
