...
On Arrow Lake 265K, AUTO dense primitive routing remains **CPU oneMKL-dominated**. The two current search hot-loop bets (iGPU CAGRA walk, NPU PQ ADC) are wired but do not beat FAISS-CPU / hnswlib. NPU ADC is now fail-closed for FP16-range saturation; the bounded AUTO path recovered CPU-equivalent recall but remained 6.82× slower than FORCE_CPU at `nprobe=8`. Lunar Lake (plan v1.0 SKU) has no tables.
...
| ovVS IVF-Flat / IVF-PQ | IVF-PQ bounded AUTO recall now matches FORCE_CPU after the ADC range guard; AUTO is still 6.82× slower at nprobe=8. The current refined ovVS/raw FAISS smoke is not a matched competitor gate. |
...
| B3 | IVF-PQ search rewrite: persistent list codes, ADC tables batched over `nq×nprobe`, iGPU variable-length scan; padded-list NPU bakeoff | T10.2–T10.4, T9.3 | **PARTIAL / PERSISTENT CSR + NUMERIC GUARD COMPLETE; BATCHING OPEN:** range-safe NPU ADC and atomic publication retain exact AUTO/CPU bounded recall parity. IVF-PQ now derives validated list offsets, IDs, and contiguous codes once at build/load/extend; unfiltered search consumes those codes directly, while filtered search copies only allowed rows. Native telemetry measured 384 direct rows and zero host list-code scratch bytes across two full-probe searches; it does not measure final candidate-ID aggregation or backend/request copies. `IPQ1` v1 remains byte-compatible and extend is transactional. Three current-code bounded smoke runs had median-of-run-median QPS 12,910.5/3,479.1 for AUTO and 74,888.8/31,815.5 for FORCE_CPU at `nprobe=2/8`; variance was material, FORCE_NPU stayed unavailable, and raw FAISS is not refinement-equivalent. Query residual/LUT construction, per-query/list dispatch, fixed buckets, request pools, iGPU scan/select, scaled ADC, and matched SIFT1M competitor evidence remain open. Evidence: `tables/arrow-lake/ivfpq-b3.md`. |
...
3. **B3** then **B4** remain the next implementation steps (batch descriptors/fixed ADC buckets after persistent IVF-PQ CSR, then RaBitQ binary GEMM).
