...
| ID | Item | Plan | Notes |
|---|---|---|---|
| B1 | SIFT1M / 1e5×768 recall–QPS harness vs FAISS-CPU and hnswlib; AUTO + FORCE_*; µJ/query | T22.1–T22.2, §8–§9 | **PARTIAL:** strict CLI/JSON harness, bounded smoke validation, independent construction/search policy, fail-closed matched SIFT1M and prefix gates, loaded-DLL fingerprints, and an opt-in CAGRA→stock-hnswlib lane are complete. The cached-worst checkpoint adds a clean repeated/interleaved CAGRA A/B curve: three old-source and three candidate processes, complete tails and package energy, exact truth, and unchanged hnswlib settings. Current CAGRA reaches 0.9609 recall and 2,549.9 QPS versus hnswlib 0.9589 and 6,746.8 QPS. Other algorithms/policies, matched FAISS comparisons, a native-versus-WSL control before any Google ScaNN comparator, and a real 100K×768 corpus report remain open. Evidence: `tables/arrow-lake/cagra-build-v1.md`, `tables/arrow-lake/cagra-search-v1.md`, `tables/arrow-lake/cagra-cached-worst-v1.md`, `tables/arrow-lake/hnsw-export.md`. |
| B2 | CAGRA search kernel T13.4: one **work-group** per query, SLM itopk, bounded hashmap (not `Seen[nq×n]`), graph-aware seeds, subgroup distance | T13.4, then T13.5–T13.6 | **PARTIAL / CACHED-WORST PROMOTED; PERFORMANCE OPEN:** work-group/query, cooperative distance, SLM candidates, and a bounded visited hash are implemented. Exact cached-worst tracking passed its gate across three old-source and three candidate SIFT1M processes: QPS improved 81.9%, 118.6%, 156.5%, and 155.7% at `32/1` batch 32, `64/2` batch 32, `128/4` batch 32, and `128/4` batch one; p50 fell 44.8–61.8% and p99 fell 43.7–61.8%. Recall was unchanged in every candidate process, the retained old-graph fixture matched IDs and distance bits across original, rebuilt-old, and final candidate DLLs, build wall changed +0.49%, and peak RSS +0.64%. Product performance still loses: hnswlib is 2.65× faster at the nearest ≥0.95 point, 4.19× near 0.989 recall, and 60.8× at the non-recall-matched batch-one point. Next gate prefix-only final selection, then stable one-pass pick and canonical frontier tiling before cooperative selection or a bounded owner/helper multi-work-group route. Evidence: `tables/arrow-lake/cagra-cached-worst-v1.md`. |
...
1. Continue promoted **B2** search work one variable at a time: exact prefix-only final selection, stable one-pass `search_width` pick, then canonical frontier tiling. Gate each against `c7e9c04` with exact old/new outputs and complete p50/p99/QPS/recall evidence; never weaken hnswlib settings. Keep cooperative selection and the owner/helper multi-work-group small-batch route separate.
