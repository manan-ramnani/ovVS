...
| B3 | IVF-PQ search rewrite: persistent list codes, ADC tables batched over `nq×nprobe`, iGPU variable-length scan; padded-list NPU bakeoff | T10.2–T10.4, T9.3 | **PARTIAL / PERSISTENT CSR + FIXED BUCKETS + AFFINE FORCE_NPU + FUSED FORCE_GPU SCAN/SELECT COMPLETE:** the C ABI and `IPQ1` v1 remain unchanged. The iGPU path scans persistent uint8 codes, applies filters, performs exact `(score,dense ordinal)` list/query selection, returns only packed shortlist positions, and fails closed; lifecycle, 32+1 block, tail-bit, boundary, atomicity, and concurrent-resource tests pass. A clean bounded three-run result retained recall 0.6375/0.946875 but FORCE_GPU reached only 3,555/2,336 QPS versus FORCE_CPU 80,665/31,885 QPS at `nprobe=2/8` (22.69×/13.65× CPU lead). AUTO is not promoted. Stage telemetry, removal of the raw-row batch cap, exact Q/T LUT factorization, persistent GPU workspaces, batched refinement, bounded/versioned NPU request caching, and matched SIFT1M evidence remain open. Evidence: `tables/arrow-lake/ivfpq-b3.md`. |
...
3. **B3** remains the next implementation step. The fused FORCE_GPU scan/select is correct but loses the bounded CPU route by 13.65× at `nprobe=8`. Add complete-call stage telemetry, replace the raw-candidate block cap with a bounded workspace-byte budget, factor query/list LUT work exactly, reuse GPU workspaces, and batch exact refinement. Keep NPU request-depth work experimental until the bounded/versioned cache and telemetry exist. Then correct and accelerate **B4** RaBitQ.
