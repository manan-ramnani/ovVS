...
| B2 | CAGRA search kernel T13.4: one **work-group** per query, SLM itopk, bounded hashmap (not `Seen[nq×n]`), graph-aware seeds, subgroup distance | T13.4, then T13.5–T13.6 | **PARTIAL:** work-group/query, cooperative distance, SLM candidates, and a bounded visited hash are implemented. Resource-local transfer counters prove direct GPU-accessible dataset/graph pointers and zero explicit index uploads in the 4,200-row regression and all 2K smoke points; SIFT1M evidence is still missing. Hash seeds and leader-serial selection/sort remain. The 2K smoke keeps recall but is roughly 10× slower than hnswlib at `itopk=32/search_width=1` and farther behind at `64/2`; the SIFT1M recall gate is still unrun. |
...
Checkpoint verification: 63/63 native tests, 6/6 CTest lanes, 31/31 benchmark-harness tests, and 6/6 SIFT-fetcher tests. Device-absent scale lanes report CTest SKIP rather than PASS.
