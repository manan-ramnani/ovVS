...
The C ABI covers brute-force, IVF-Flat/PQ/RaBitQ, CAGRA, NN-Descent, Vamana, ScaNN, HNSW-from-CAGRA, all-neighbors, filters, dynamic batcher, k-means, SLINK, spectral, PQ/SQ/binary, PCA, pairwise, k-selection. Python ctypes + NumPy/DLPack ingest exist. Most algorithm tests remain toy-sized (**n=24–80**); bounded NN-Descent GPU regressions cover 4,097 and 16,384 rows. A strict matched SIFT1M CAGRA/hnswlib quality gate exists; the latest full-scale run failed by 48.46 recall percentage points and predates the current seed, graph-optimizer, and NN-Descent changes. A current-code 100K SIFT-prefix preflight is valid intermediate evidence only: CAGRA recall was 0.9718 versus hnswlib median 0.9510, but hnswlib median search/build remained 5.64×/5.16× faster.

On Arrow Lake 265K, AUTO dense primitive routing remains **CPU oneMKL-dominated**. The two remaining hardware bets (iGPU CAGRA walk, NPU PQ ADC) are wired but do not beat FAISS-CPU / hnswlib. Lunar Lake (plan v1.0 SKU) has no tables.
...
| ID | Item | Plan | Notes |
|---|---|---|---|
| B1 | SIFT1M / 1e5×768 recall–QPS harness vs FAISS-CPU and hnswlib; AUTO + FORCE_*; µJ/query | T22.1–T22.2, §8–§9 | **PARTIAL:** strict CLI/JSON harness, bounded smoke validation, independent construction/search policy, a fail-closed matched SIFT1M CAGRA recall gate, and a contract-checked 100K SIFT-prefix preflight are complete. The full gate produced valid negative evidence: CAGRA 0.4057 recall@10 versus hnswlib 0.8903. Three current-code 100K preflights completed with exact FAISS truth: CAGRA recall was 0.9718 in every run versus hnswlib 0.9504–0.9525; median QPS was 3,923.6/22,142.6, build wall 8.686/1.682 s, and isolated peak RSS 398.4/201.2 MiB. They had no energy measurement and cannot close B1. Full SIFT1M curves/energy, the other algorithms/policies, and a real 100K×768 corpus report remain open. The pinned SIFT file is local and not committed. |
| B2 | CAGRA search kernel T13.4: one **work-group** per query, SLM itopk, bounded hashmap (not `Seen[nq×n]`), graph-aware seeds, subgroup distance | T13.4, then T13.5–T13.6 | **PARTIAL / FULL QUALITY GATE FAILED:** work-group/query, cooperative distance, SLM candidates, and a bounded visited hash are implemented. SIFT1M counters prove `192/192/0/0` walk calls/direct calls/upload calls/upload bytes: there were zero explicit index uploads, although driver-managed shared-memory migration was not measured. At the nominal public M=16/effort=32 point, the pre-change full gate measured CAGRA recall@10 0.4057 versus hnswlib 0.8903 (gap 0.4846; allowed 0.0200); the two graph topologies are not identical. Query-content seeds now make CPU/GPU results invariant to batch partition and query order, and the host build now uses deterministic detour-rank/reverse-interleave optimization instead of nearest-only truncation. Current code reached 0.9718 versus hnswlib median 0.9510 on the 100K prefix with all 192 searches per run attributed to GPU, but hnswlib was 5.64× faster by median QPS and the prefix is not the SIFT1M verdict. The seeds are still not graph-aware; effort controls and leader-serial selection/sort remain before QPS tuning. |
| B3 | IVF-PQ search rewrite: persistent list codes, ADC tables batched over `nq×nprobe`, iGPU variable-length scan; padded-list NPU bakeoff | T10.2–T10.4, T9.3 | `prim_pq_adc` on NPU is real. `ovvsIvfPqSearch` rebuilds residual + tables + packed codes **on the host per query per list**. That is why FAISS is ~6× faster on 2k points. After persistent codes land, compare a bounded set of padded ADC buckets and reusable request-pool depths 1/2/4; preserve exact recall/settings and report queueing, tail latency, memory, energy, and end-to-end wall. |
...
| B5 | NN-Descent iGPU local-join + bloom for n≫4096 | T12 | **PARTIAL / DOWNSTREAM FULL GATE NOT RERUN:** n≤4096 remains exact kNN. Larger L2/L2-sqrt/IP/cosine builds now use deterministic cached-distance NEW/OLD tagging, bounded forward/reverse samples, NEW–NEW and NEW–OLD reciprocal proposals, target-owned merges, and exact changed/pending-NEW convergence accounting on SYCL. Bounded sampled exact overlap improved from 0.7266 to 0.9727 at n=4,097 and from 0.2832 to 0.8750 at n=16,384; both cases exhausted their 6/4-iteration budgets with pending NEW edges. Across three current-code 100K SIFT-prefix runs, AUTO build wall was 8.672–8.800 s (8.686 s median), isolated CAGRA peak process RSS was 398.2–398.5 MiB, and downstream recall was 0.9718, but the final optimizer primitive remained CPU. The latest SIFT1M result remains the pre-change 0.4057 failure. Estimated direct-dataset device ownership at SIFT1M/K64 is 1.116 GiB, but worst-case work remains large and no current-code SIFT1M practicality, recall, or peak-process-memory claim is made. Full FORCE_GPU build still rejects until T13.3; the explicit IVF-PQ initializer independently retains its `nlist≤8` per-row-search scale defect. |
...
B1, B2, and B5 remain partial. The full SIFT1M gate still must be rerun on current code; the older 0.4057 result remains the latest full-scale measurement. Harness verification is 57/57 tests plus Python compilation; details are in `tables/arrow-lake/bench-recall-qps.md`, `tables/arrow-lake/cagra-transfer-b2.md`, and `tables/arrow-lake/nndescent-b5.md`.
