...
| B3 | IVF-PQ search rewrite: persistent list codes, ADC tables batched over `nq×nprobe`, iGPU variable-length scan; padded-list NPU bakeoff | T10.2–T10.4, T9.3 | **PARTIAL / NUMERIC CORRECTNESS GUARDED:** real SIFT residual ADC sums reached ~85k–100k and 128-row NPU tiles saturated at 65,504, causing bounded AUTO recall 0.3781 versus FORCE_CPU 0.9469. A conservative per-LUT bound plus atomic output publication now makes unsafe AUTO fall back to CPU and FORCE_NPU fail unavailable; the final rerun restored exact recall parity at 0.946875. AUTO remained 3,441.7 QPS versus CPU 23,463.9 (6.82× deficit) at `nprobe=8`. `ovvsIvfPqSearch` still rebuilds residuals, tables, IDs, and codes on the host per query/list. Infer-only depth-4 pooling was 2.30× depth 1 and larger fixed buckets amortized launches, but neither is end-to-end evidence. Next: persistent CSR list codes, scaled ADC validation, batched work descriptors, fixed buckets, then bounded request pools. Evidence: `tables/arrow-lake/ivfpq-b3.md`. |
...
3. **B3** then **B4** are the next implementation steps (persistent/batched IVF-PQ ADC, then RaBitQ binary GEMM).
