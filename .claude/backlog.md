...
On Arrow Lake 265K, AUTO dense primitive routing remains **CPU oneMKL-dominated**. The two current search hot-loop bets (iGPU CAGRA walk, NPU PQ ADC) are wired but do not beat matched FAISS-CPU / hnswlib gates. NPU ADC is fail-closed for FP16-range saturation; persistent storage plus fixed-bucket batching preserve CPU-equivalent bounded recall, but clean-code AUTO remains 1.40× slower than FORCE_CPU at `nprobe=8`. That retained end-to-end artifact predates the bounded FORCE_NPU affine transform and reports its wide-range forced lane unavailable; the later transformed primitive is correct in bounded tests but 73–106× slower than CPU. Lunar Lake (plan v1.0 SKU) has no tables.
...
| B3 | IVF-PQ search rewrite: persistent list codes, ADC tables batched over `nq×nprobe`, iGPU variable-length scan; padded-list NPU bakeoff | T10.2–T10.4, T9.3 | **PARTIAL / PERSISTENT CSR + FIXED BUCKETS + FORCED AFFINE RANGE TRANSFORM COMPLETE:** validated list-major storage, transactional lifecycle, bounded descriptors, reused refinement workspaces, and whole-search publication are implemented without changing the C ABI or `IPQ1` v1. Fixed buckets `{128,256,512,1024,2048}` use adaptive capacity-bounded NPU Gather requests. FORCE_NPU can subtract each LUT subspace minimum, scale a remaining span to 60,000 when the scale is at least 0.5, validate the output, and restore the bias atomically. A deterministic scale-0.588 fixture retained 32/32 top candidates with 27.25 maximum score error; a scale-0.029 case rejected. This is not a speed win: three-process hot primitive medians left NPU about 106× slower than CPU at 131,072 candidates and 73.3× slower at 524,288. AUTO therefore keeps unsafe LUTs on CPU. Earlier clean-code SIFT-prefix search retained recall 0.6375/0.946875 and left CPU 1.41×/1.40× faster at `nprobe=2/8`. Bounded/versioned request caching, stage telemetry, iGPU scan/select, and matched SIFT1M evidence remain open. Evidence: `tables/arrow-lake/ivfpq-b3.md`. |
...
3. **B3** remains the next implementation step for bounded/versioned request caching, stage telemetry, and iGPU fused scan/select. The forced affine ADC lane is implemented but loses its current primitive bakeoff, so request-depth work remains experimental. Then correct and accelerate **B4** RaBitQ.
