...
Evidence: `tables/arrow-lake/cagra-build-v1.md`, `tables/arrow-lake/bench-recall-qps.md`, `tables/arrow-lake/hnsw-export.md`, `tables/arrow-lake/npu-igpu-escape-routes.md`, `tables/arrow-lake/gemm_large.json`, and `docs/hw-split.md`.
...
| ID | Item | Plan | Notes |
|---|---|---|---|
| B1 | SIFT1M / 1e5×768 recall–QPS harness vs FAISS-CPU and hnswlib; AUTO + FORCE_*; µJ/query | T22.1–T22.2, §8–§9 | **PARTIAL:** strict CLI/JSON harness, bounded smoke validation, independent construction/search policy, fail-closed matched SIFT1M and prefix gates, loaded-DLL fingerprints, and an opt-in CAGRA→stock-hnswlib lane are complete. Three clean full SIFT1M points pass recall closeness at 0.9036 versus hnswlib median 0.8911, while CAGRA delivers 30.35% of hnswlib QPS and builds 1.302× slower. Three clean SIFT100K export runs validated a fully entry-reachable stock-hnswlib graph, but the hybrid lost native build and nearest comparable-recall search. Full SIFT1M curves/energy, other algorithms/policies, the ≥0.95 target point, and a real 100K×768 corpus report remain open. Evidence: `tables/arrow-lake/cagra-build-v1.md`, `tables/arrow-lake/hnsw-export.md`. |
| B2 | CAGRA search kernel T13.4: one **work-group** per query, SLM itopk, bounded hashmap (not `Seen[nq×n]`), graph-aware seeds, subgroup distance | T13.4, then T13.5–T13.6 | **PARTIAL / T13.4 RECALL GATE PASSED; PERFORMANCE OPEN:** work-group/query, cooperative distance, SLM candidates, and a bounded visited hash are implemented. Three clean SIFT1M processes reached CAGRA 0.9036 versus hnswlib median 0.8911; every gap passes the maximum 0.0200 limit. Each process reported all 192 searches on GPU and `192/192/0/0` walk/direct/upload-call/upload-byte deltas. Query-content seeds are batch/order invariant, and the host build uses deterministic detour-rank/reverse-interleave optimization. The result remains below the 0.95 product target; median throughput is 3,465 versus 11,417 QPS and amortized p99 is 1.090 versus 0.101 ms/query. A static kernel audit estimates that the 128-thread work-group leaves lane 0 with about 221K serial comparisons and reaches up to 7,361 barriers per canonical query. The next bounded kernel experiment is deterministic subgroup-16 cooperative selection for L2/dim128/degree16/itopk32/width1, promoted only with exact IDs/recall, at least 10% SIFT1M batch-32 p50 improvement, p99 no more than 5% worse, and no structural-resource regression. |
...
1. Reduce the measured **GPU NN-Descent initializer** wall without changing graph quality: the clean SIFT1M median is 90.444 s of a 100.093 s build, with 2,458 submissions, 1,820 waits, and 459 D2H calls. Start with synchronization/readback removal; compare interleaved complete walls and preserve exact structural counters or document intentional changes.
2. Move the 9.034-second host graph optimize/prune/merge stage to the iGPU only as a separately attributable change. The measured export-only route is parked.
...
The clean build-stage evidence now attributes the first target to GPU NN-Descent synchronization/readback, followed by host optimize/prune/merge. The remaining critical path is measured initializer reduction, subgroup-cooperative CAGRA search, hierarchical/packed iGPU PQ, and size-gated CPU+iGPU batching. No BIOS, firmware, driver, or power setting changed.
