...
The C ABI covers brute-force, IVF-Flat/PQ/RaBitQ, CAGRA, NN-Descent, Vamana, ScaNN, HNSW-from-CAGRA, all-neighbors, filters, dynamic batcher, k-means, SLINK, spectral, PQ/SQ/binary, PCA, pairwise, k-selection. Python ctypes + NumPy/DLPack ingest exist. Most algorithm tests remain toy-sized (**n=24–80**); bounded NN-Descent GPU regressions cover 4,097 and 16,384 rows. A strict matched SIFT1M CAGRA/hnswlib quality gate now exists; the latest measured run failed by 48.46 recall percentage points and predates the current seed, graph-optimizer, and NN-Descent changes.
...
| ID | Item | Plan | Notes |
|---|---|---|---|
| B1 | SIFT1M / 1e5×768 recall–QPS harness vs FAISS-CPU and hnswlib; AUTO + FORCE_*; µJ/query | T22.1–T22.2, §8–§9 | **PARTIAL:** strict CLI/JSON harness, bounded smoke validation, independent construction/search policy, and a fail-closed matched SIFT1M CAGRA recall gate are complete. The first gate used the full 1M-vector index and 1,000 queries and produced valid negative evidence: CAGRA 0.4057 recall@10 versus hnswlib 0.8903. Full SIFT1M curves/energy, the other algorithms/policies, and a real 100K×768 corpus report remain open. The pinned SIFT file is local and not committed. |
| B2 | CAGRA search kernel T13.4: one **work-group** per query, SLM itopk, bounded hashmap (not `Seen[nq×n]`), graph-aware seeds, subgroup distance | T13.4, then T13.5–T13.6 | **PARTIAL / QUALITY GATE FAILED:** work-group/query, cooperative distance, SLM candidates, and a bounded visited hash are implemented. SIFT1M counters prove `192/192/0/0` walk calls/direct calls/upload calls/upload bytes: there were zero explicit index uploads, although driver-managed shared-memory migration was not measured. At the nominal public M=16/effort=32 point, CAGRA recall@10 was 0.4057 versus hnswlib 0.8903 (gap 0.4846; allowed 0.0200); the two graph topologies are not identical. Query-content seeds now make CPU/GPU results invariant to batch partition and query order, and the host build now uses deterministic detour-rank/reverse-interleave optimization instead of nearest-only truncation. The gate predates both changes and has not been rerun; the seeds are still not graph-aware. Initializer quality, effort controls, and leader-serial selection/sort remain before QPS tuning. |
...
| B5 | NN-Descent iGPU local-join + bloom for n≫4096 | T12 | **PARTIAL / DOWNSTREAM GATE NOT RERUN:** n≤4096 remains exact kNN. Larger L2/L2-sqrt/IP/cosine builds now use deterministic cached-distance NEW/OLD tagging, bounded forward/reverse samples, NEW–NEW and NEW–OLD reciprocal proposals, target-owned merges, and exact changed/pending-NEW convergence accounting on SYCL. Bounded sampled exact overlap improved from 0.7266 to 0.9727 at n=4,097 and from 0.2832 to 0.8750 at n=16,384; both cases exhausted their 6/4-iteration budgets with pending NEW edges. The latest SIFT1M CAGRA result remains the pre-change 0.4057 failure. Estimated direct-dataset device ownership at SIFT1M/K64 is 1.116 GiB, but worst-case work remains large and no SIFT1M practicality, recall, or peak-process-memory claim is made. Host CAGRA optimization is still CPU, so full FORCE_GPU build rejects until T13.3. The explicit IVF-PQ initializer independently retains its `nlist≤8` per-row-search scale defect. |
