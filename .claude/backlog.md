...
1. Continue **B2/B9** from the measured search curve. Begin with exact cached-worst candidate tracking, then profile deterministic SIMD16 selection and a multi-work-group/query small-batch route; never weaken hnswlib settings. Promote only on repeated complete p50/p99/QPS/recall and structural counters.
...
The clean build-stage evidence attributes the first construction target to GPU NN-Descent synchronization/readback, followed by host optimize/prune/merge. The later same-index curve moves the primary search target to exact cached-worst candidate tracking and then subgroup-cooperative/small-batch CAGRA work. Hierarchical/packed iGPU PQ and size-gated CPU+iGPU batching remain on the critical path. No BIOS, firmware, driver, or power setting changed.
