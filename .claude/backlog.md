...
| B5 | NN-Descent iGPU local-join + bloom for n≫4096 | T12 | **PARTIAL / TWO SYNC DESIGNS REJECTED; T12.5 OPEN:** n≤4096 remains exact kNN. Larger L2/L2-sqrt/IP/cosine builds use deterministic cached-distance NEW/OLD tagging, bounded forward/reverse samples, reciprocal proposals, target-owned merges, and exact changed/pending-NEW accounting on SYCL. Three clean SIFT1M AUTO constructions completed in 99.874–100.525 s (100.093 s median) within 2,343.2 MiB median isolated peak RSS and supported 0.9036 downstream CAGRA recall. The GPU initializer contributes 90.444 s (90.36%) of median complete wall; host optimize/prune/merge contributes 9.034 s. Each build reports 1,098 kernels, 2,458 submissions, 1,820 waits, 459 D2H calls, zero H2D, and six iterations without convergence. Fixed-`N` device-resident active counts regressed complete wall 8.91%; a convergence-only single-work-group reduction then cut D2H bytes 27.27% but regressed complete wall 2.14%. Both were reverted. Next initialize `heads[N]` once per iteration and have unique target-owned consumers reset only touched rows, while preserving dynamic ranges and the original convergence path. T12.5 IVF-PQ-initializer comparison, K64 scale, and T13.3 remain open. Evidence: `tables/arrow-lake/cagra-build-v1.md`, `tables/arrow-lake/nndescent-active-count-v1.md`, `tables/arrow-lake/nndescent-convergence-reduction-v1.md`. |
...
2. Reduce the measured **GPU NN-Descent initializer** wall without changing graph quality: the clean SIFT1M median is 90.444 s of a 100.093 s build. Do not revive fixed-`N` active-count consumers (+8.91% complete wall) or the single-work-group convergence reduction (+2.14%). Preserve dynamic ranges, scalar readbacks, and the original convergence path. Replace the 444 per-chunk `heads[N]` fills with one initialization per iteration plus target-owned consumer resets; compare fresh parent/candidate complete walls and exact counters before any promotion.
