...
The C ABI covers brute-force, IVF-Flat/PQ/RaBitQ, CAGRA, NN-Descent, Vamana, ScaNN, HNSW-from-CAGRA, all-neighbors, filters, dynamic batcher, k-means, SLINK, spectral, PQ/SQ/binary, PCA, pairwise, k-selection. Python ctypes + NumPy/DLPack ingest exist. Most algorithm tests remain toy-sized (**n=24–80**); bounded NN-Descent GPU regressions cover 4,097 and 16,384 rows. The current-code matched SIFT1M CAGRA gate passes the plan's recall-closeness condition: 0.9036 recall@10 versus hnswlib 0.8915, a 0.0121 advantage. This is a 0.4979 absolute recovery from the pre-change CAGRA result, but both lanes remain below the 0.95 product target and hnswlib search was 6.02× faster in the load-contaminated diagnostic run. The exported single-layer graph now loads, searches, saves, and reloads through stock hnswlib; three clean SIFT100K diagnostics found it fully entry-reachable but slower to build and slower at comparable recall than native hnswlib, so it is interoperability evidence rather than an acceleration route. A frozen 256-byte CAGRA build telemetry ABI now separates dataset copy/validation, initializer, optimizer/prune/merge, and graph materialization while recording call-local NN-Descent GPU work. No clean SIFT100K or SIFT1M build-stage artifact has been accepted yet.
...
Evidence: `tables/arrow-lake/bench-recall-qps.md`, `tables/arrow-lake/hnsw-export.md`, `tables/arrow-lake/npu-igpu-escape-routes.md`, `tables/arrow-lake/gemm_large.json`, and `docs/hw-split.md`.
...
| ID | Item | Plan | Notes |
|---|---|---|---|
| B1 | SIFT1M / 1e5×768 recall–QPS harness vs FAISS-CPU and hnswlib; AUTO + FORCE_*; µJ/query | T22.1–T22.2, §8–§9 | **PARTIAL:** strict CLI/JSON harness, bounded smoke validation, independent construction/search policy, a fail-closed matched SIFT1M CAGRA recall gate, a contract-checked 100K SIFT-prefix preflight, and an opt-in CAGRA→stock-hnswlib lane are complete. The full SIFT1M point passes recall closeness at 0.9036 versus hnswlib 0.8915, but hnswlib was 6.02× faster in a load-contaminated diagnostic. Three clean SIFT100K export runs completed 9/9 lanes and validated a fully entry-reachable stock-hnswlib graph; the hybrid still lost native hnswlib at the nearest measured comparable-recall points and took 5.779× as long to become searchable. Full SIFT1M curves/energy, the other algorithms/policies, the ≥0.95 target point, and a real 100K×768 corpus report remain open. Evidence: `tables/arrow-lake/hnsw-export.md`. |
| B2 | CAGRA search kernel T13.4: one **work-group** per query, SLM itopk, bounded hashmap (not `Seen[nq×n]`), graph-aware seeds, subgroup distance | T13.4, then T13.5–T13.6 | **PARTIAL / T13.4 RECALL GATE PASSED; PERFORMANCE OPEN:** work-group/query, cooperative distance, SLM candidates, and a bounded visited hash are implemented. On current code the full SIFT1M matched point reached CAGRA 0.9036 versus hnswlib 0.8915; `hnswlib-CAGRA=-0.0121` passes the maximum 0.0200 gap. All 192 searches reported GPU and `192/192/0/0` walk/direct/upload-call/upload-byte deltas. Query-content seeds are batch/order invariant, and the host build uses deterministic detour-rank/reverse-interleave optimization. The result remains below the 0.95 product target. A static kernel audit estimates that the 128-thread work-group leaves lane 0 with about 221K serial comparisons and reaches up to 7,361 barriers per canonical query. The next bounded kernel experiment is deterministic subgroup-16 cooperative selection for L2/dim128/degree16/itopk32/width1, promoted only with exact IDs/recall, at least 10% SIFT1M batch-32 p50 improvement, p99 no more than 5% worse, and no structural-resource regression. |
...
| B5 | NN-Descent iGPU local-join + bloom for n≫4096 | T12 | **PARTIAL / BUILD TELEMETRY COMPLETE; T12.5 OPEN:** n≤4096 remains exact kNN. Larger L2/L2-sqrt/IP/cosine builds use deterministic cached-distance NEW/OLD tagging, bounded forward/reverse samples, reciprocal proposals, target-owned merges, and exact changed/pending-NEW convergence accounting on SYCL. Current-code SIFT1M AUTO construction completed in 185.3 s within 2,342.5 MiB isolated peak RSS and supported 0.9036 downstream CAGRA recall. The additive 256-byte V1 telemetry is resource-local, cumulative, saturating, and success-only; it records the four named build stages, exact logical copy/payload bytes, initializer-final device, NN-Descent iterations/convergence, allocations, explicit transfers, kernels, submissions, literal waits, and peak algorithm-owned USM. The final optimizer primitive remains CPU. Next run clean revision/DLL-bound SIFT100K then SIFT1M stage measurements before moving the measured bottleneck to iGPU. Standalone convergence, T12.5 IVF-PQ-initializer comparison, K64 scale, and T13.3 remain open. |
...
1. Run the landed **CAGRA build-stage telemetry** from a clean, revision-bound DLL: three SIFT100K diagnostic processes, then three SIFT1M processes only if ABI, exact bytes, GPU initializer attribution, structural counters, recall, and output invariants all pass. Record the DLL hash and environment; the prefix run is not a promotion result.
2. Use that evidence to move the measured CAGRA/NN-Descent construction or graph optimize/prune/merge bottleneck to the iGPU where complete build plus unchanged stock-hnswlib CPU search improves. The measured export-only route is parked.
3. Continue **B2/B9** with the profiled SIMD16 CAGRA kernel portfolio; never weaken hnswlib settings. Promote only on repeated complete p50/p99/QPS/recall and structural counters.
...
7. In parallel, finish **B1** full curves/energy and the real 100K×768 corpus. Bake off **B8** only when Lunar Lake hardware exists, then continue B9–B20 and B21–B32 in plan order.
...
Python exposes `build_algo={nndescent,ivf_pq,iterative}` with a backward-compatible NN-Descent default. Ordinary profiles report a missing old-library symbol explicitly; the fixed SIFT100K/SIFT1M modes fail closed unless one fresh AUTO build proves an instrumented GPU NN-Descent initializer with exact geometry and valid structural counters. Verification is 94/94 native tests with zero skips, 8/8 configured CTest lanes, and 83/83 harness tests. This is instrumentation and correctness evidence, not a construction speedup. A clean DLL-hashed SIFT100K/SIFT1M measurement is still required before attributing or optimizing the historical 185.3-second build.
...
The critical path therefore moves to explicit CAGRA build-stage attribution, measured iGPU construction/optimizer work, subgroup-cooperative CAGRA search, hierarchical/packed iGPU PQ, and size-gated CPU+iGPU batching. No BIOS, firmware, driver, or power setting changed.
