...
| B5 | NN-Descent iGPU local-join + bloom for n≫4096 | T12 | **PARTIAL / T13.3 BOUNDED GPU OPTIMIZER PROMOTED; T12.5 OPEN:** n≤4096 remains exact kNN. Larger L2/L2-sqrt/IP/cosine builds use deterministic cached-distance NEW/OLD tagging, bounded forward/reverse samples, reciprocal proposals, target-owned merges, and exact changed/pending-NEW accounting on SYCL. Exact T13.3 optimize/prune/merge now runs on the iGPU through intermediate degree 64. Three AUTO SIFT1M builds completed in 91.791–100.018 s (91.987 s median), with 0.641 s median optimizer wall, 2,269.8 MiB median peak RSS, exact graph bytes versus the frozen CPU parent, and unchanged 0.9036 recall plus initializer counters. Full FORCE_GPU NN-Descent CAGRA build is available for supported shapes. The initializer still contributes 90.883 s and all runs report 1,098 kernels, 2,458 submissions, 1,820 waits, 459 D2H calls, zero H2D, and six iterations without convergence. Fixed-`N` active counts (+8.91% complete wall), convergence reduction (+2.14%), target-owned head reset (+5.55%), and producer→copy event chaining (+8.96%) remain reverted. Further initializer work requires device-side phase evidence and less algorithmic work; separately measure a device-resident initializer-to-optimizer handoff. T12.5 IVF-PQ-initializer comparison and K64 scale remain open. Evidence: `tables/arrow-lake/cagra-gpu-optimize-v1.md`, `tables/arrow-lake/cagra-build-v1.md`, and the four rejection reports. |
...
This is not a product acceleration claim. Energy was disabled, the SIFT1M
artifacts are quality gates rather than full B1 curves, and ovVS still builds
1.182× slower than unchanged hnswlib in the candidate cohort. The remaining
90.883-second initializer requires device-side phase profiling and a separately
measured resident initializer-to-optimizer handoff. Evidence:
`tables/arrow-lake/cagra-gpu-optimize-v1.md`.
