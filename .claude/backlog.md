...
Three independent pre-commit bounded SIFT-prefix runs retained recall 0.6375/0.946875 at `nprobe=2/8`. AUTO median-of-run-median QPS was 62,573.3/23,491.4 versus FORCE_CPU 85,538.6/32,533.6. At `nprobe=8`, the checkpoint ratio is 1.38× instead of 9.14×; AUTO's median ratio between checkpoints is 6.75×. This is not an isolated batching, hardware, or competitor win: the artifacts are dirty, the wide-range fixture still makes FORCE_NPU unavailable, AUTO's final primitive is CPU, raw FAISS is unrefined and recalls only 0.553125/0.6375, package energy is disabled, and the comparison with the preceding checkpoint is not simultaneous. The ovVS peak process RSS range was 140.9–141.3 MiB.
