...
On Arrow Lake 265K, AUTO dense primitive routing remains **CPU oneMKL-dominated**. The current search hot-loop bets are real device paths but do not beat matched FAISS-CPU / hnswlib gates. NPU ADC is fail-closed for FP16-range saturation; persistent storage plus fixed-bucket batching preserve CPU-equivalent bounded recall. Unfiltered IVF-PQ avoids the dense 8-byte candidate-ID copy and resolves only the exact-refinement shortlist. A fused FORCE_GPU variable-list scan/select now removes the dense ADC score array, but a clean bounded run remains 13.65× slower than FORCE_CPU at `nprobe=8`; AUTO therefore remains on the prior measured route. The bounded FORCE_NPU transform is correct in synthetic tests but 73–106× slower than CPU. Lunar Lake (plan v1.0 SKU) has no tables.
...
| B3 | IVF-PQ search rewrite: persistent list codes, ADC tables batched over `nq×nprobe`, iGPU variable-length scan; padded-list NPU bakeoff | T10.2–T10.4, T9.3 | **PARTIAL / COMPLETE-CALL TELEMETRY LANDED:** persistent CSR, synchronous fixed buckets, bounded affine FORCE_NPU, shortlist-only ID resolution, and fused FORCE_GPU scan/select are complete. A frozen additive 200-byte telemetry ABI now publishes success-only resource-local stage/counter snapshots; timed harness deltas fail closed on incompatible, backwards, incomplete, or internally inconsistent records. Three clean bounded runs at `6a0f4c5` retained recall 0.6375/0.946875. At `nprobe=8`, FORCE_CPU reached 32,074 QPS and spent a median 73.6% of native call wall constructing LUTs; FORCE_GPU reached 2,284 QPS and spent 45.5% in fused ADC plus 40.0% in final per-query TopK, with 99 allocations, 120 kernel submissions, and 230 waits per call. AUTO is not promoted. Exact Q/T LUT factorization with a cutoff-gap certificate, a workspace-byte block budget, persistent GPU workspaces, batched/fused refinement, bounded/versioned NPU request caching, and matched SIFT1M evidence remain open. `IPQ1` v1 is unchanged. Evidence: `tables/arrow-lake/ivfpq-b3.md`. |
...
3. **B3** remains the next implementation step. Complete-call telemetry identifies exact query/list LUT construction as 73.6% of the bounded CPU `nprobe=8` call and fused ADC plus final per-query TopK as 85.5% of the GPU call. Implement exact Q/T factorization with a cutoff-gap certificate and direct-LUT fallback; replace the raw-candidate cap with a workspace-byte budget; then persist GPU workspaces and batch/fuse exact refinement. Keep NPU request-depth work experimental until its cache is bounded/versioned and joined end-to-end wall wins. Then correct and accelerate **B4** RaBitQ.
