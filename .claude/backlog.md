...
| B5 | NN-Descent iGPU local-join + bloom for n≫4096 | T12 | **PARTIAL / DOWNSTREAM QUALITY FAILED:** n≤4096 remains exact kNN; larger L2/L2-sqrt/IP/cosine builds use a deterministic, double-buffered SYCL sampled local join with SLM candidates/Bloom and honest forced-policy failures. The first AUTO SIFT1M CAGRA build completed in 830.0 s, but downstream recall was only 0.4057. Host CAGRA optimization now has the intended detour-rank, capped reverse graph, and deterministic interleave semantics; it is unmeasured at scale and the SYCL T13.3 port remains open, so full FORCE_GPU CAGRA build still rejects. Real NN-Descent NEW/OLD/reverse joins, convergence, tuning, and peak-RSS evidence remain open. The explicit IVF-PQ initializer independently retains its `nlist≤8` per-row-search scale defect. |
