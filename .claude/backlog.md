...
| B5 | NN-Descent iGPU local-join + bloom for n≫4096 | T12 | **PARTIAL / THREE SYNC DESIGNS REJECTED; T12.5 OPEN:** n≤4096 remains exact kNN. Larger L2/L2-sqrt/IP/cosine builds use deterministic cached-distance NEW/OLD tagging, bounded forward/reverse samples, reciprocal proposals, target-owned merges, and exact changed/pending-NEW accounting on SYCL. Three clean SIFT1M AUTO constructions completed in 99.874–100.525 s (100.093 s median) within 2,343.2 MiB median isolated peak RSS and supported 0.9036 downstream CAGRA recall. The GPU initializer contributes 90.444 s (90.36%) of median complete wall; host optimize/prune/merge contributes 9.034 s. Each build reports 1,098 kernels, 2,458 submissions, 1,820 waits, 459 D2H calls, zero H2D, and six iterations without convergence. Fixed-`N` active counts regressed complete wall 8.91%; a convergence-only reduction regressed it 2.14% despite 27.27% fewer D2H bytes; and target-owned head reset regressed it 5.55% despite 17.82% fewer submissions. All were reverted. Next retain dynamic consumers and bulk fills and chain only producer completion into the active-count copy. T12.5 IVF-PQ-initializer comparison, K64 scale, and T13.3 remain open. Evidence: `tables/arrow-lake/cagra-build-v1.md`, `tables/arrow-lake/nndescent-active-count-v1.md`, `tables/arrow-lake/nndescent-convergence-reduction-v1.md`, `tables/arrow-lake/nndescent-heads-reset-v1.md`. |
...
2. Reduce the measured **GPU NN-Descent initializer** wall without changing graph quality: the clean SIFT1M median is 90.444 s of a 100.093 s build. Do not revive fixed-`N` active-count consumers (+8.91% complete wall), the single-work-group convergence reduction (+2.14%), or target-owned head reset (+5.55%). Preserve dynamic ranges, bulk head fills, scalar readbacks, and the original convergence path. As one bounded variable, make each active-count copy depend on its producer event and remove only the redundant queue-wide producer barrier; compare fresh parent/candidate complete walls and exact counters before any promotion.
