...
The C ABI covers brute-force, IVF-Flat/PQ/RaBitQ, CAGRA, NN-Descent, Vamana, ScaNN, HNSW-from-CAGRA, all-neighbors, filters, dynamic batcher, k-means, SLINK, spectral, PQ/SQ/binary, PCA, pairwise, k-selection. Python ctypes + NumPy/DLPack ingest exist. Most algorithm tests remain toy-sized (**n=24–80**); bounded NN-Descent GPU regressions cover 4,097 and 16,384 rows. A strict matched SIFT1M CAGRA/hnswlib quality gate now exists and has run; it currently fails by 48.46 recall percentage points.
...
| ID | Item | Plan | Notes |
|---|---|---|---|
| B1 | SIFT1M / 1e5×768 recall–QPS harness vs FAISS-CPU and hnswlib; AUTO + FORCE_*; µJ/query | T22.1–T22.2, §8–§9 | **PARTIAL:** strict CLI/JSON harness, bounded smoke validation, independent construction/search policy, and a fail-closed matched SIFT1M CAGRA recall gate are complete. The first full-corpus gate produced valid negative evidence: CAGRA 0.4057 recall@10 versus hnswlib 0.8903. Full SIFT1M curves/energy, the other algorithms/policies, and a real 100K×768 corpus report remain open. The pinned SIFT file is local and not committed. |
| B2 | CAGRA search kernel T13.4: one **work-group** per query, SLM itopk, bounded hashmap (not `Seen[nq×n]`), graph-aware seeds, subgroup distance | T13.4, then T13.5–T13.6 | **PARTIAL / QUALITY GATE FAILED:** work-group/query, cooperative distance, SLM candidates, and a bounded visited hash are implemented. SIFT1M counters prove `192/192/0/0` walk calls/direct calls/upload calls/upload bytes, so transfers are not the quality failure. At matched M=16/effort=32, CAGRA recall@10 was 0.4057 versus hnswlib 0.8903 (gap 0.4846; allowed 0.0200). Hash/query-aware seeds, leader-serial selection/sort, and builder/search quality diagnosis remain before QPS tuning. |
...
| B5 | NN-Descent iGPU local-join + bloom for n≫4096 | T12 | **PARTIAL / DOWNSTREAM QUALITY FAILED:** n≤4096 remains exact kNN; larger L2/L2-sqrt/IP/cosine builds use a deterministic, double-buffered SYCL sampled local join with SLM candidates/Bloom and honest forced-policy failures. The first AUTO SIFT1M CAGRA build completed in 830.0 s and finished with host prune, but downstream matched-point recall was only 0.4057. Convergence/new-edge scheduling, reverse-candidate quality, tuning, peak-RSS evidence, and T13.3 GPU prune remain open; full FORCE_GPU CAGRA build still rejects. The explicit IVF-PQ initializer independently retains its `nlist≤8` per-row-search scale defect. |
| B6 | Wire `OVVS_POLICY_HETERO` as stage overlap (NPU ADC ∥ iGPU walk), not a third GEMM backend | T20, `docs/hw-split.md` | Currently equals AUTO. Implement a bounded stage queue with reusable requests/events and per-stage wall/queue/byte telemetry. OpenVINO GenAI continuous batching is a scheduler reference, not evidence that ANN or NPU overlap is fast. |

### P1 — v1.0 quality and SKU coverage

| ID | Item | Plan | Notes |
|---|---|---|---|
| B7 | NPU index home: dataset / codebook as L0 tensor (`create_l0_host_tensor` or non-regressing Constant). Close wall vs DPU (5.3 vs ~45 ms on 1e5×32×768) | NPU contract, `npu-gemm-dpu-vs-wall.md` | Sticky Parameter + memcpy is current. OpenVINO 2026 documents NPU `RemoteTensor`/`create_l0_host_tensor`, native-handle and CPU-address import, and optional strided IO; the measured build is OpenVINO 2025.3, so upgrade/probe/driver validation is required before this is an implementation claim. Baking B as Constant regressed wall. Arrow Lake AUTO GEMM stays CPU until an end-to-end SKU table flips. |
...
Checkpoint verification: 63/63 native tests, 6/6 CTest lanes, 45/45 benchmark-harness tests, and 6/6 SIFT-fetcher tests. Device-absent scale lanes report CTest SKIP rather than PASS.
