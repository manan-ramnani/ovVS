...
Evidence: `tables/arrow-lake/cagra-build-v1.md`, `tables/arrow-lake/cagra-search-v1.md`, `tables/arrow-lake/cagra-frontier-v1.md`, `tables/arrow-lake/bench-recall-qps.md`, `tables/arrow-lake/hnsw-export.md`, `tables/arrow-lake/npu-igpu-escape-routes.md`, `tables/arrow-lake/gemm_large.json`, and `docs/hw-split.md`.
...
| ID | Item | Plan | Notes |
|---|---|---|---|
| B1 | SIFT1M / 1e5×768 recall–QPS harness vs FAISS-CPU and hnswlib; AUTO + FORCE_*; µJ/query | T22.1–T22.2, §8–§9 | **PARTIAL:** strict CLI/JSON harness, bounded smoke validation, independent construction/search policy, fail-closed matched SIFT1M and prefix gates, loaded-DLL fingerprints, and an opt-in CAGRA→stock-hnswlib lane are complete. The current combined prefix/frontier checkpoint adds a clean repeated/interleaved CAGRA A/B curve: three frozen-baseline and three candidate processes, complete tails and package energy, exact truth, and unchanged hnswlib settings. Current CAGRA reaches 0.9609 recall and 2,762.9 QPS versus hnswlib 0.9602 and 6,678.4 QPS. Other algorithms/policies, matched FAISS comparisons, a native-versus-WSL control before any Google ScaNN comparator, and a real 100K×768 corpus report remain open. Evidence: `tables/arrow-lake/cagra-build-v1.md`, `tables/arrow-lake/cagra-search-v1.md`, `tables/arrow-lake/cagra-frontier-v1.md`, `tables/arrow-lake/hnsw-export.md`. |
| B2 | CAGRA search kernel T13.4: one **work-group** per query, SLM itopk, bounded hashmap (not `Seen[nq×n]`), graph-aware seeds, subgroup distance | T13.4, then T13.5–T13.6 | **PARTIAL / FRONTIER PROMOTED; PERFORMANCE OPEN:** work-group/query, cooperative distance, SLM candidates, and a bounded visited hash are implemented. Exact cached-worst tracking first delivered 81.9–156.5% QPS gains. Prefix-only final sorting and canonical 64-ID frontier tiling then passed a separate six-process gate against that frozen baseline: QPS improved 8.0%, 8.0%, 9.0%, and 10.9% at `32/1` batch 32, `64/2` batch 32, `128/4` batch 32, and `128/4` batch one; p50 fell 7.3–9.7% and p99 fell 7.2–11.0%. Recall stayed unchanged, the cross-DLL fixture matched ID and distance bytes, build wall changed -0.63%, and peak RSS by less than 0.01%. Explicit barriers fell 7,361→593, 27,009→1,185, and 103,169→2,369, but every accepted distance still executes its group reduction. Product performance still loses: hnswlib is 2.42× faster at the nearest ≥0.95 point, 3.84× near 0.989 recall, and 54.8× at the non-recall-matched batch-one point. A stable one-pass pick was exact but rejected after regressing three of four effort points. Next gate cooperative integer/min-location selection, then a bounded owner/helper multi-work-group route. Evidence: `tables/arrow-lake/cagra-frontier-v1.md`. |
...
1. Continue promoted **B2** search work one variable at a time from `892252b`: cooperative integer/min-location selection, then a bounded owner/helper multi-work-group small-batch route. Gate each against the frozen frontier DLL with exact old/new outputs and complete p50/p99/QPS/recall evidence; never weaken hnswlib settings. The exact stable one-pass pick is rejected evidence and must not be revived without a materially different design.
...
The clean build-stage evidence attributes the first construction target to GPU NN-Descent synchronization/readback, followed by host optimize/prune/merge. The later same-index curve moved the primary search target to cached-worst tracking, which subsequently passed its repeated complete-wall gate. Prefix-only publication sorting and canonical frontier tiling then passed their own repeated gate; the exact stable one-pass pick was rejected after a negative admission screen. Search work now proceeds to cooperative integer/min-location selection and a separately gated owner/helper small-batch experiment. Hierarchical/packed iGPU PQ and size-gated CPU+iGPU batching remain on the critical path. No BIOS, firmware, driver, or power setting changed.
