...
The C ABI covers brute-force, IVF-Flat/PQ/RaBitQ, CAGRA, NN-Descent, Vamana, ScaNN, HNSW-from-CAGRA, all-neighbors, filters, dynamic batcher, k-means, SLINK, spectral, PQ/SQ/binary, PCA, pairwise, k-selection. Python ctypes + NumPy/DLPack ingest exist. Most algorithm tests remain toy-sized (**n=24–80**); bounded NN-Descent GPU regressions cover 4,097 and 16,384 rows. Three clean current-code SIFT1M CAGRA gates pass the plan's recall-closeness condition at the low-effort point: 0.9036 recall@10 versus hnswlib median 0.8911. A later clean same-index curve reaches the separate 0.95 target at CAGRA `itopk=64/search_width=2` (0.9609 versus hnswlib 0.9589 at `ef=64`), demonstrating that measured graph can support the target. Performance remains negative: at that comparable-quality point CAGRA delivers 17.51% of hnswlib QPS, has 5.31× higher batch p99, and uses 3.74× more whole-package energy/query. The first cached-worst walk change is implemented and exactness-qualified, but has no complete-wall result yet and is not an acceleration claim. The exported single-layer graph now loads, searches, saves, and reloads through stock hnswlib; three clean SIFT100K diagnostics found it fully entry-reachable but slower to build and slower at comparable recall than native hnswlib, so it is interoperability evidence rather than an acceleration route. The frozen 256-byte CAGRA build telemetry ABI attributes 90.36% of the clean 100.09-second SIFT1M median build to GPU NN-Descent and 9.03 seconds to host optimize/prune/merge. All three builds hit the six-iteration cap rather than convergence.
...
Evidence: `tables/arrow-lake/cagra-build-v1.md`, `tables/arrow-lake/cagra-search-v1.md`, `tables/arrow-lake/bench-recall-qps.md`, `tables/arrow-lake/hnsw-export.md`, `tables/arrow-lake/npu-igpu-escape-routes.md`, `tables/arrow-lake/gemm_large.json`, and `docs/hw-split.md`.
...
| ID | Item | Plan | Notes |
|---|---|---|---|
| B1 | SIFT1M / 1e5×768 recall–QPS harness vs FAISS-CPU and hnswlib; AUTO + FORCE_*; µJ/query | T22.1–T22.2, §8–§9 | **PARTIAL:** strict CLI/JSON harness, bounded smoke validation, independent construction/search policy, fail-closed matched SIFT1M and prefix gates, loaded-DLL fingerprints, and an opt-in CAGRA→stock-hnswlib lane are complete. Three clean low-effort SIFT1M points pass recall closeness at 0.9036 versus hnswlib median 0.8911. One later same-index SIFT1M CAGRA/hnswlib benchmark invocation includes tails and package energy and reaches 0.9609 versus 0.9589 at comparable recall, but CAGRA delivers only 17.51% of hnswlib QPS. This invocation's two sequential isolated lane processes are diagnostic rather than promotion evidence. Three clean SIFT100K export runs validated a fully entry-reachable stock-hnswlib graph, but the hybrid lost native build and nearest comparable-recall search. Repeated/interleaved CAGRA curves, other algorithms/policies, FAISS comparisons, and a real 100K×768 corpus report remain open. Evidence: `tables/arrow-lake/cagra-build-v1.md`, `tables/arrow-lake/cagra-search-v1.md`, `tables/arrow-lake/hnsw-export.md`. |
| B2 | CAGRA search kernel T13.4: one **work-group** per query, SLM itopk, bounded hashmap (not `Seen[nq×n]`), graph-aware seeds, subgroup distance | T13.4, then T13.5–T13.6 | **PARTIAL / ≥0.95 RECALL POINT OBSERVED; PERFORMANCE OPEN:** work-group/query, cooperative distance, SLM candidates, and a bounded visited hash are implemented. Three clean SIFT1M low-effort processes reached CAGRA 0.9036 versus hnswlib median 0.8911 and passed the maximum 0.0200 closeness gap with direct zero-upload GPU walks. A later benchmark invocation's CAGRA lane reached 0.9609 at `64/2` versus hnswlib 0.9589 at `ef=64`, but QPS was 1,169 versus 6,675, batch p99 29.126 versus 5.482 ms, and package energy 18,298 versus 4,892 µJ/query. At about 0.989 recall the QPS deficit widened to 10.82×, and batch-one `128/4` exposed a 156× raw QPS deficit against the higher-recall hnswlib `ef=256` point. A static kernel audit estimates that the 128-thread work-group leaves lane 0 with about 221K serial comparisons and reaches up to 7,361 barriers per canonical query. Exact cached-worst candidate tracking is now implemented as an unpromoted candidate: it preserves the old strict ordering and rescans only after an accepted replacement, without adding SLM, barriers, allocations, submissions, waits, or transfers. Exact tests and a local serialized-graph diagnostic produced by the pre-change DLL match IDs and distance bits at `32/1`, `64/2`, and `128/4`; complete-wall measurement remains open. Promotion still requires at least 10% SIFT1M batch-32 p50 improvement, p99 no more than 5% worse, exact IDs/recall, and no structural-resource regression. If it passes, profile deterministic SIMD16 cooperative selection and a measured multi-work-group/query small-batch route separately. Evidence: `tables/arrow-lake/cagra-search-v1.md`. |
...
1. Measure the exact cached-worst **B2** candidate against the clean same-index curve. Retain it only if repeated complete p50/p99/QPS/recall and structural counters pass the documented gate; never weaken hnswlib settings. Then profile deterministic SIMD16 selection and a multi-work-group/query small-batch route as separate changes.
...
The clean build-stage evidence attributes the first construction target to GPU NN-Descent synchronization/readback, followed by host optimize/prune/merge. The later same-index curve moves the primary search target to the now-implemented cached-worst candidate and then subgroup-cooperative/small-batch CAGRA work. The cached-worst change is exactness-qualified but still awaits complete-wall measurement. Hierarchical/packed iGPU PQ and size-gated CPU+iGPU batching remain on the critical path. No BIOS, firmware, driver, or power setting changed.
...
The 2,048-row `32/1` non-exhaustive regression now requires bitwise GPU batch/single/reorder distances and exact IDs. The 4,200-row scale test also compares repeated output bytes. A local serialized-graph diagnostic produced by the pre-change DLL matched candidate IDs and distance bits at `32/1`, `64/2`, and `128/4` under the rebuilt DLL; its files remain in ignored `out/` rather than canonical evidence. Verification is 94/94 native tests, all eight configured CTest lanes after rerunning the sandbox-denied interop subprocess outside that boundary, and 87/87 benchmark-harness tests. This is an exactness checkpoint only: no complete SIFT1M timing has run, so there is no latency, throughput, energy, or acceleration claim.
