...
On Arrow Lake 265K, AUTO dense primitive routing remains **CPU oneMKL-dominated**. The two current search hot-loop bets (iGPU CAGRA walk, NPU PQ ADC) are wired but do not beat matched FAISS-CPU / hnswlib gates. NPU ADC is fail-closed for FP16-range saturation; persistent storage plus fixed-bucket batching preserve CPU-equivalent bounded recall. Unfiltered IVF-PQ now avoids the dense 8-byte candidate-ID copy and resolves only the exact-refinement shortlist; a clean matched bounded smoke improved `nprobe=8` AUTO/CPU QPS by 1.10×/1.12×, but current CPU remains 1.51× faster than AUTO. The bounded FORCE_NPU transform is correct in synthetic tests but 73–106× slower than CPU. Lunar Lake (plan v1.0 SKU) has no tables.
...
| ovVS IVF-Flat / IVF-PQ | IVF-PQ bounded AUTO recall matches FORCE_CPU after the ADC range guard. Shortlist-only ID resolution improved clean bounded `nprobe=8` AUTO/CPU QPS by 1.10×/1.12×, but CPU remains 1.51× faster. The refined ovVS/raw FAISS smoke is not a matched competitor gate. |
...
| B3 | IVF-PQ search rewrite: persistent list codes, ADC tables batched over `nq×nprobe`, iGPU variable-length scan; padded-list NPU bakeoff | T10.2–T10.4, T9.3 | **PARTIAL / PERSISTENT CSR + FIXED BUCKETS + AFFINE FORCE_NPU + SHORTLIST-ONLY IDS COMPLETE:** validated list-major storage, transactional lifecycle, bounded descriptors, reused refinement workspaces, whole-search publication, and lossless unfiltered ID resolution are implemented without changing the C ABI or `IPQ1` v1. Unfiltered search no longer copies an `int64` ID per scored candidate; it resolves only the `krefine` shortlist through ordered descriptors. A clean three-run bounded A/B retained recall 0.6375/0.946875 and improved `nprobe=8` AUTO QPS from 19,573.1 to 21,627.5 (1.10×) and CPU from 29,157.2 to 32,679.7 (1.12×); CPU is still 1.51× faster. FORCE_NPU's bounded affine lane remains correctness-only: the scaled primitive was 73–106× slower than CPU. Bounded/versioned request caching, stage telemetry, iGPU fused scan/select, and matched SIFT1M evidence remain open. Evidence: `tables/arrow-lake/ivfpq-b3.md`. |
...
3. **B3** remains the next implementation step for bounded/versioned request caching, stage telemetry, and iGPU fused scan/select. Persistent codes, fixed buckets, the forced affine ADC lane, and shortlist-only ID resolution are complete. The NPU lane loses its current primitive bakeoff, so request-depth work remains experimental. Then correct and accelerate **B4** RaBitQ.
