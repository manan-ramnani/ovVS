...
| B5 | NN-Descent iGPU local-join + bloom for n≫4096 | T12 | **PARTIAL / FIRST SYNC DESIGN REJECTED; T12.5 OPEN:** n≤4096 remains exact kNN. Larger L2/L2-sqrt/IP/cosine builds use deterministic cached-distance NEW/OLD tagging, bounded forward/reverse samples, reciprocal proposals, target-owned merges, and exact changed/pending-NEW accounting on SYCL. Three clean SIFT1M AUTO constructions completed in 99.874–100.525 s (100.093 s median) within 2,343.2 MiB median isolated peak RSS and supported 0.9036 downstream CAGRA recall. The GPU initializer contributes 90.444 s (90.36%) of median complete wall; host optimize/prune/merge contributes 9.034 s. Each build reports 1,098 kernels, 2,458 submissions, 1,820 waits, 459 D2H calls, zero H2D, and six iterations without convergence. A fixed-`N` device-resident active-count candidate reduced calls/submissions/waits but regressed a fresh SIFT1M parent by 8.91% complete wall and 9.85% initializer wall, so it was reverted. Next reduce only the two full convergence-array readbacks while preserving dynamic consumer ranges. T12.5 IVF-PQ-initializer comparison, K64 scale, and T13.3 remain open. Evidence: `tables/arrow-lake/cagra-build-v1.md`, `tables/arrow-lake/nndescent-active-count-v1.md`. |
...
2. Reduce the measured **GPU NN-Descent initializer** wall without changing graph quality: the clean SIFT1M median is 90.444 s of a 100.093 s build, with 2,458 submissions, 1,820 waits, and 459 D2H calls. Do not revive the rejected fixed-`N` active-count route, which regressed complete wall 8.91%. Preserve dynamic consumer ranges and replace the two full convergence-array readbacks per iteration with an exact device reduction plus one small validated host summary; compare fresh parent/candidate complete walls and record all intentional counter changes.
