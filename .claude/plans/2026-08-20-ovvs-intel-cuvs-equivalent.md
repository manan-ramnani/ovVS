...
- **T13.3** Optimize/prune SYCL: detour-count rank reorder, fixed-degree prune, capped reverse graph, and deterministic forward/reverse interleave. Nearest-only truncation is not CAGRA graph optimization. The bounded Arrow Lake path is promoted through intermediate degree 64; the decision log governs policy and fallback.
...
| NN-Descent build | exact primitive path through n=4096; bounded sampled iGPU NEW/OLD join above it | **B5 partial; GPU optimizer and cooperative minima promoted, T12.5 open:** deterministic cached-distance NEW/OLD tagging, bounded forward/reverse samples, NEW–NEW and NEW–OLD reciprocal proposals, target-owned merges, and exact changed/pending-NEW accounting execute on SYCL. Exact optimize/prune/merge runs on iGPU through intermediate degree 64. The proposal join now computes up to 96 row/column minima cooperatively while preserving ascending scan and lane-0 emission order. Three AUTO SIFT1M candidate builds have a 61.000 s median and 2,272.9 MiB median peak RSS; the initializer falls from the current 90.883 s parent median to 60.164 s and the current optimizer median is 0.655 s. Parent/candidate graph bytes match exactly at SIFT100K and SIFT1M, downstream recall remains 0.9036, and full FORCE_GPU NN-Descent CAGRA build is available for supported shapes. All runs still hit the six-iteration cap with about 18.5% changed/pending edges, so standalone convergence, K64 scale, and the T12.5 comparison against IVF-PQ initialization remain open. Four synchronization designs and two coordinate-staging layouts remain negative evidence. Reprofile the promoted join before another one-variable change and separately measure a resident initializer-to-optimizer handoff. Evidence: `tables/arrow-lake/nndescent-cooperative-minima-v1.md`, `tables/arrow-lake/cagra-gpu-optimize-v1.md`, and the rejection reports. |
| CAGRA build telemetry | frozen additive V1 snapshot; external API wall remains canonical | `ovvsResourcesCagraBuildStatsV1` publishes 31 resource-local cumulative counters only after a successful build is ready to publish. Four named stages, exact logical bytes, initializer-final device, and call-local GPU NN-Descent lifecycle counters are separated from the final selected optimizer attribution. `total_wall_ns` is the inner wall through persistent graph materialization and excludes telemetry merge, caller-handle publication, and serialization. Fixed benchmark modes require a fresh, fully instrumented AUTO GPU NN-Descent initializer and a fingerprinted loaded library matching explicit `--library`. This enables attribution; it is not by itself an acceleration result. |
| CAGRA graph optimization | exact SYCL detour-rank prune plus capped reverse-edge interleave through intermediate degree 64 | AUTO/GPU_IF_FASTER/HETERO use the GPU for supported shapes; cap/device absence may use the exact CPU path, while selected GPU runtime/correctness failure is fail-closed. FORCE_GPU completes only with a GPU NN-Descent initializer and supported GPU optimizer. The isolated promotion reduced optimizer wall 92.90%; after the later initializer promotion its current three-run median is 0.655 s inside a 61.000 s complete build. Exact serialized graph bytes remain unchanged. The current M=16/intermediate-32 graph still reaches 0.9609 on the same-index `64/2` search point. Retain 16/32 as the low-memory baseline and treat 32/64 as a later controlled quality/scale experiment. Evidence: `tables/arrow-lake/cagra-gpu-optimize-v1.md` and `tables/arrow-lake/nndescent-cooperative-minima-v1.md`. |
...
| Remaining work after v0.2 ABI | Kernel quality + scale, not more algorithm C ABI names | 2026-08-29 status: symbols cover the plan’s algorithm list; the additive IVF-PQ and CAGRA telemetry ABIs bring the export count to 112. Three clean SIFT1M low-effort CAGRA gates pass recall closeness. Cached-worst tracking, prefix-only publication sorting, canonical frontier tiling, cooperative multi-pick selection, bounded T13.3 GPU graph optimization, and exact NN-Descent cooperative minima are promoted. The latest search gate retains 0.9609 recall and 3,243.5 QPS versus packaged hnswlib 6,784.5 at comparable recall. Cooperative minima reduces repeated complete SIFT1M construction from the current 91.978 s parent median to 61.000 s; this beats the packaged stock hnswlib cohort by 20.87%, but that control's float-distance arithmetic uses legacy SSE. A source-pinned MSVC `/arch:AVX2` builder, semantically validated hash-matched loader, and targeted batch-cap 256/1024 points now exist and pass reproducible import/static-SIMD validation; repeated complete SIFT1M results remain mandatory before a full-speed claim. The four-workgroup owner/helper small-batch route is parked after a fail-closed Arrow Lake capacity result of one group versus four required; the broader one-workgroup/SIMD16 portfolio remains open. Four synchronization designs and two coordinate-staging layouts are rejected; reprofile the promoted join before further initializer changes and measure a resident handoff. HNSW export interoperability is valid but loses build and nearest comparable-recall search at SIFT100K. Persistent IVF-PQ storage, buckets, numeric guards, shortlist-only IDs, fused FORCE_GPU scan/select, and both telemetry contracts are complete; Q/T and Arrow Lake NPU escape routes are negative within their documented scopes. Hierarchical/packed iGPU PQ, size-gated CPU+iGPU batching, other B1 algorithms/policies, a same-OS ScaNN control, and the real 100K×768 corpus remain open. Journal: `.claude/backlog.md`. |
| hnswlib AVX2 comparator evidence | reproducible static/import candidate; performance open | Two fresh roots produced the same wheel and module from the pinned v0.8.0 commit/tree and dependency set. The harness validates semantic provenance and exact full-profile CAGRA/HNSW point multisets. Static AVX/YMM instructions are not active-cycle attribution; repeated complete SIFT1M results remain required. Evidence: `tables/arrow-lake/evidence/hnswlib-avx2-build-v1/README.md`. |
...
- **FAISS/hnswlib pip:** `faiss-cpu` 1.15.0 and `hnswlib` 0.8.0 import. SIFT **n=2000** artifacts remain smoke only. Three clean strict SIFT1M low-effort pairs pass recall closeness: CAGRA 0.9036 versus hnswlib median about 0.891. The promoted cooperative-pick checkpoint raises current CAGRA `64/2` to 3,243.5 QPS at 0.9609 recall versus packaged hnswlib 6,784.5 QPS at 0.9593; the 2.09× deficit remains negative product evidence. Cooperative minima puts ovVS construction ahead of that packaged cohort, but the installed Windows extension uses legacy-SSE float-distance arithmetic and one search thread for batches 1 and 32. The separate hnswlib 0.8.0 `/arch:AVX2` module is now source/build-dependency pinned and hash-verified; the harness includes batch-cap 256/1024 points and records actual batch/effective-thread provenance. No replacement performance result exists yet. AUTO/CPU brute beats FAISS only on the bounded slice; the legacy NPU brute result is invalid due FP16-range saturation and now fails closed; IVF paths remain behind FAISS. Other policies/algorithms, matched FAISS curves, a same-OS ScaNN control, and the real 100K×768 corpus keep B1 open.
...
- **NN-Descent SYCL B5 checkpoint:** n>4096 L2-family/IP/cosine construction uses cached-distance NEW/OLD tags, bounded sampled forward/reverse lists, NEW–NEW and NEW–OLD reciprocal proposals, deterministic target-owned merges, and exact changed/pending-NEW accounting. T13.3 exact GPU optimize/prune/merge is promoted through intermediate degree 64. The later exact cooperative-minima join reduces three-run SIFT1M median complete build 91.978→61.000 s and initializer wall 90.883→60.164 s, with exact 100K/1M graph bytes, unchanged 0.9036 recall/lifecycle counters, +0.04% search QPS, +0.03% p99, and +0.14% peak RSS. All runs still exhaust six iterations. Four synchronization designs and the bank-safe T64/NEW-only staging layouts are rejected and restored. Reprofile the current join before another one-variable implementation and separately measure a resident handoff. This does not close T12.5: standalone convergence, the IVF-PQ-init comparison, and K64 scale remain open. Evidence: `tables/arrow-lake/nndescent-cooperative-minima-v1.md`, `tables/arrow-lake/cagra-gpu-optimize-v1.md`, and the rejection reports.
