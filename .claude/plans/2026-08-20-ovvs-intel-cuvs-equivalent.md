...
| CAGRA walk | `prim_graph_walk`: fused SYCL when built; adaptive host fallback otherwise | **B2 ≥0.95 recall point observed; throughput open:** one work-group/query, cooperative distance, SLM candidate/expanded state, traversal-budget global visited hash, chunked hash allocation, and strict device USM are implemented. Three clean current SIFT1M low-effort processes reached CAGRA recall@10 0.9036 versus hnswlib median 0.8911 and passed the maximum 0.0200 closeness gap with direct zero-upload GPU walks. A later complete same-index curve reaches 0.9609 at `itopk=64/search_width=2` versus hnswlib 0.9589 at `ef=64`, demonstrating that the measured graph supports the ≥0.95 recall point. CAGRA nevertheless delivers only 17.51% of hnswlib QPS at that point, with 5.31× higher batch p99 and 3.74× higher package energy/query; at about 0.989 recall the QPS deficit is 10.82×. Exact cached-worst tracking is implemented and exactness-qualified without changing structural resources, but complete-wall performance remains unmeasured. Gate it before profiling deterministic SIMD16 cooperative selection, a multi-work-group/query small-batch route, graph-aware landmarks, and explicit effort controls. Forced GPU rejection never host-fallbacks; FORCE_NPU graph walk is unavailable. Evidence: `tables/arrow-lake/cagra-search-v1.md`. |
...
- **FAISS/hnswlib pip:** `faiss-cpu` 1.15.0 and `hnswlib` 0.8.0 import. SIFT **n=2000** artifacts remain smoke only. Three clean strict SIFT1M low-effort pairs pass recall closeness: CAGRA 0.9036 versus hnswlib median 0.8911. One later same-index CAGRA/hnswlib benchmark invocation reaches 0.9609 versus 0.9589 near the ≥0.95 target, but hnswlib is 5.71× faster and CAGRA uses 3.74× more package energy/query. Its two sequential isolated lane processes form the first selected CAGRA curve, not repeated promotion evidence or the complete multi-algorithm B1 report. AUTO/CPU brute beats FAISS only on the bounded slice; the legacy NPU brute result is invalid due FP16-range saturation and now fails closed; IVF paths remain behind FAISS.
