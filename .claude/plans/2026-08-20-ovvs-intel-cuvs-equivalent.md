...
| NN-Descent build | exact primitive path through n=4096; bounded sampled iGPU NEW/OLD join above it | **B5 partial; T13.3 bounded GPU optimizer promoted, T12.5 open:** deterministic cached-distance NEW/OLD tagging, bounded forward/reverse samples, NEW–NEW and NEW–OLD reciprocal proposals, target-owned merges, and exact changed/pending-NEW accounting execute on SYCL. Exact optimize/prune/merge now also runs on the iGPU through intermediate degree 64. Three AUTO SIFT1M candidate builds have a 91.987 s median and 2,269.8 MiB median peak RSS; the optimizer falls from the clean 9.034 s CPU baseline to 0.641 s, while the GPU initializer remains 90.883 s. Parent/candidate graph bytes match exactly at SIFT100K and SIFT1M, downstream recall remains 0.9036, and full FORCE_GPU NN-Descent CAGRA build is available for supported shapes. All runs still hit the six-iteration cap with about 18.5% changed/pending edges, so standalone convergence, K64 scale, and the T12.5 comparison against IVF-PQ initialization remain open. Four synchronization changes remain negative evidence: device-resident active-count/fixed-`N` consumers regressed complete build 8.91%; convergence reduction 2.14%; target-owned head reset 5.55%; and producer→copy event chaining 8.96%. Further initializer work requires device-side phase evidence, less algorithmic work, and a separately measured resident initializer-to-optimizer handoff. Evidence: `tables/arrow-lake/cagra-gpu-optimize-v1.md`, `tables/arrow-lake/cagra-build-v1.md`, and the four rejection reports. |
...
| CAGRA graph optimization | exact SYCL detour-rank prune plus capped reverse-edge interleave through intermediate degree 64 | AUTO/GPU_IF_FASTER/HETERO use the GPU for supported shapes; cap/device absence may use the exact CPU path, while selected GPU runtime/correctness failure is fail-closed. FORCE_GPU completes only with a GPU NN-Descent initializer and supported GPU optimizer. Three AUTO SIFT1M builds reduce optimizer wall 92.90% and complete build 8.10%, with exact serialized graph bytes and 3.13% lower peak RSS. The current M=16/intermediate-32 graph still reaches 0.9609 on the same-index `64/2` search point. Retain 16/32 as the low-memory baseline and treat 32/64 as a later controlled quality/scale experiment. Evidence: `tables/arrow-lake/cagra-gpu-optimize-v1.md`. |
...
- **NN-Descent SYCL B5 checkpoint:** n>4096 L2-family/IP/cosine construction uses cached-distance NEW/OLD tags, bounded sampled forward/reverse lists, NEW–NEW and NEW–OLD reciprocal proposals, deterministic target-owned merges, and exact changed/pending-NEW accounting. T13.3 exact GPU optimize/prune/merge is promoted through intermediate degree 64. Three AUTO SIFT1M candidate builds have a 91.987 s median and 2,269.8 MiB median peak RSS; optimizer wall is 0.641 s versus the clean 9.034 s CPU baseline, while the GPU initializer contributes 90.883 s. Exact parent/candidate graph bytes and downstream recall 0.9036 are retained. All runs still exhaust six iterations. Fixed-`N` active-count consumers (+8.91% complete wall), convergence reduction (+2.14%), target-owned `heads` reset (+5.55%), and producer→copy event chaining (+8.96%) are rejected and restored. Production retains dynamic consumer ranges, bulk head fills, scalar readbacks, and the original convergence path. Further initializer work requires device-side phase evidence, less algorithmic work, and a separately measured resident handoff. This does not close T12.5: standalone convergence, the IVF-PQ-init comparison, and K64 scale remain open. Evidence: `tables/arrow-lake/cagra-gpu-optimize-v1.md`, `tables/arrow-lake/cagra-build-v1.md`, and the four rejection reports.
