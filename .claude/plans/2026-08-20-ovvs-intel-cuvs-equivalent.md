...
| CAGRA walk | `prim_graph_walk`: fused SYCL when built; adaptive host fallback otherwise | **B2 quality gate failed:** one work-group/query, cooperative distance, SLM candidate/expanded state, traversal-budget global visited hash, chunked hash allocation, and strict device USM are implemented. The strict SIFT1M gate recorded CAGRA recall@10 0.4057 versus hnswlib 0.8903 at the nominal public M=16/effort=32 point; the graph topologies are not identical. All 192 calls reported GPU and `192/192/0/0` walk/direct/upload-call/upload-byte deltas. The fixed walk scores at most 3,584 unique rows/query (0.3584% of SIFT1M). Query-content hashing now gives batch/order-invariant CPU/GPU seed streams, but the gate predates that fix and has not been rerun. Graph-aware landmarks, explicit effort controls, and quality closure remain before QPS tuning. Forced GPU rejection never host-fallbacks; FORCE_NPU graph walk is unavailable. |
| NN-Descent build | exact primitive path through n=4096; bounded sampled iGPU NEW/OLD join above it | **B5 partial; downstream gate not rerun:** deterministic cached-distance NEW/OLD tagging, bounded forward/reverse samples, NEW–NEW and NEW–OLD reciprocal proposals, target-owned merges, and exact changed/pending-NEW convergence accounting execute on SYCL. Bounded sampled exact overlap is 0.9727 at n=4,097/K=8 and 0.8750 at n=16,384/K=16, but both fixtures exhaust their iteration budgets. The latest SIFT1M gate used the previous initializer and remains a 0.4057 measured failure, not a current-code result. Estimated SIFT1M/K64 owned device allocation is 1.116 GiB with a device-accessible dataset, while worst-case work remains large. Host CAGRA optimization is still CPU, so full FORCE_GPU build rejects until the T13.3 SYCL port. Evidence: `tables/arrow-lake/nndescent-b5.md`. |
...
- **FAISS/hnswlib pip:** `faiss-cpu` 1.15.0 and `hnswlib` 0.8.0 import. SIFT **n=2000** artifacts remain smoke only. The first strict SIFT1M pair is valid negative quality evidence: CAGRA 0.4057 recall@10 versus hnswlib 0.8903. It is not the full B1 curve/energy report. AUTO/CPU brute beats FAISS only on the bounded slice; the legacy NPU brute result is invalid due FP16-range saturation and now fails closed; IVF paths remain behind FAISS.
...
- **NN-Descent SYCL B5 checkpoint:** n>4096 L2-family/IP/cosine construction now uses cached-distance NEW/OLD tags, bounded sampled forward/reverse lists, NEW–NEW and NEW–OLD reciprocal proposals, deterministic target-owned merges, and exact changed/pending-NEW accounting. Five-process medians were 1,022.4 ms at 4,097 rows (0.9727 sampled exact overlap) and 1,931.4 ms at 16,384 rows (0.8750); both fixtures exhausted their 6/4-iteration budgets. The added join work is slower than the outgoing-only checkpoint but materially improves bounded graph quality. The existing SIFT1M failure measured the old initializer and nearest-only host prune; it predates this rewrite and the detour-rank/reverse optimizer. No SIFT1M practicality, recall, or performance improvement is claimed. Evidence: `tables/arrow-lake/nndescent-b5.md`.
