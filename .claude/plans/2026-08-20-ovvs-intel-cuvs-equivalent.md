...
| Remaining work after v0.2 ABI | Kernel quality + scale, not more algorithm C ABI names | 2026-08-29 status: symbols cover the plan’s algorithm list; the additive IVF-PQ and CAGRA telemetry ABIs bring the export count to 112. The SIFT1M CAGRA point passes recall closeness at 0.9036 versus 0.8915, but hnswlib was 6.02× faster in a load-contaminated diagnostic and the ≥0.95 target is open. HNSW export interoperability is valid but loses build and nearest comparable-recall search at SIFT100K. Persistent IVF-PQ storage, buckets, numeric guards, shortlist-only IDs, fused FORCE_GPU scan/select, and both telemetry contracts are complete; Q/T and Arrow Lake NPU escape routes are negative within their documented scopes. Next are clean DLL-bound CAGRA stage measurements, measured iGPU construction/optimizer work, a SIMD16 CAGRA search portfolio, hierarchical/packed iGPU PQ, and size-gated CPU+iGPU batching while full B1 curves/energy remain open. Journal: `.claude/backlog.md`. |
