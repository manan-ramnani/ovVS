...
- **IVF-PQ NPU ADC (correctness guarded, persistent CSR and synchronous batching complete):** `prim_pq_adc_batch` runs only when every LUT's conservative accumulation bound is below 65,504 and every request-owned output validates; invalid codes and forced-device failures publish nothing. Build/load/extend derive validated list-major codes. Search preserves query/probe/list order in bounded blocks and stages the complete result. Equal `{128,256,512,1024,2048}` buckets pack into adaptive depth-one Gather requests; live tests cover multi-task packing, split-tail regrouping, and bucket 2,048. Three pre-commit bounded SIFT-prefix runs retained exact AUTO/CPU recall parity and changed the `nprobe=8` checkpoint ratio from 9.14× to 1.38×, but the artifacts are dirty, FORCE_NPU remained unavailable, and no isolated batching, competitor, or energy win is claimed. Scaling, bounded/versioned request caches, depth 2/4 bakeoffs, and iGPU scan/select remain. Evidence: `tables/arrow-lake/ivfpq-b3.md`.
