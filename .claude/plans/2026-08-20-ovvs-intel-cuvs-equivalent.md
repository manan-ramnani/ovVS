...
| CAGRA walk | `prim_graph_walk`: fused SYCL when built; adaptive host fallback otherwise | **B2 T13.4 recall-closeness gate passed; throughput open:** one work-group/query, cooperative distance, SLM candidate/expanded state, traversal-budget global visited hash, chunked hash allocation, and strict device USM are implemented. The current full SIFT1M matched point reached CAGRA recall@10 0.9036 versus hnswlib 0.8915 at M=16/effort=32, passing the maximum 0.0200 gap. All 192 calls reported GPU with `192/192/0/0` transfer deltas. The fixed walk scores at most 3,584 unique rows/query (0.3584% of SIFT1M); hnswlib was 6.02× faster in the load-contaminated diagnostic run. The result is below the separate ≥0.95 target. Graph-aware landmarks, explicit effort controls, and leader-serial selection/sort remain. Forced GPU rejection never host-fallbacks; FORCE_NPU graph walk is unavailable. |
| NN-Descent build | exact primitive path through n=4096; bounded sampled iGPU NEW/OLD join above it | **B5 partial; CAGRA-vs-hnswlib gate passed, T12.5 open:** deterministic cached-distance NEW/OLD tagging, bounded forward/reverse samples, NEW–NEW and NEW–OLD reciprocal proposals, target-owned merges, and exact changed/pending-NEW convergence accounting execute on SYCL. The current SIFT1M AUTO build completed in 185.3 s within 2,342.5 MiB isolated peak process RSS and supported 0.9036 downstream CAGRA recall, passing the matched hnswlib-closeness gate. Standalone convergence and the T12.5 comparison against IVF-PQ initialization remain open. Host CAGRA optimization is still CPU, so full FORCE_GPU build rejects until the T13.3 SYCL port. K64 scale remains unproven. Evidence: `tables/arrow-lake/nndescent-b5.md`. |
| CAGRA graph optimization | detour-rank prune plus capped reverse-edge interleave | The current M=16/intermediate-32 full gate passes recall closeness but reaches only 0.9036. Record initializer and post-prune recall/connectivity separately; test 32/64 as the next quality point without discarding the low-memory 16/32 configuration. |
...
| NPU PQ ADC numeric contract | Raw execution requires `sum_m max_code(abs(lut[m, code])) < 65,504`; FORCE_NPU may affine-center and scale a remaining span to 60,000 only at scale ≥0.5; AUTO keeps the unsafe CPU fallback | Real SIFT residual LUT sums reached roughly 85k–100k and full 128-row tiles saturated at 65,504, collapsing bounded AUTO recall to 0.3781 versus CPU 0.9469. The raw guard restored parity. The bounded transform subtracts subspace minima, restores their sum after validated inference, and rejects wider transforms atomically. A scale-0.588 fixture retained 32/32 top candidates, but the transformed NPU primitive was 73–106× slower than CPU in current diagnostic shapes. It is FORCE_NPU correctness coverage, not an AUTO route. Evidence: `tables/arrow-lake/ivfpq-b3.md`. |
| IVF-PQ ADC batching | bounded ordered query/list descriptors; fixed NPU buckets `{128,256,512,1024,2048}`; fused iGPU scan/select | Equal NPU buckets pack into adaptive power-of-two-capacity `Gather(axis=1,batch_dims=1)` requests with request-owned tensors; only real rows are published. FORCE_GPU scans persistent uint8 codes, applies filters, and emits only an exact per-query shortlist without a dense score buffer. Search preserves query/probe/list-row ordering and whole-call publication. Complete-call telemetry at bounded `nprobe=8` attributes 73.6% of CPU wall to LUT construction, while GPU fused ADC plus final per-query TopK consumes 85.5% and incurs 99 allocations/120 kernels/230 waits per call. AUTO remains on CPU. Exact Q/T factorization with a cutoff-gap certificate, a workspace-byte budget, and batched/fused GPU refinement are next. NPU depth 1/2/4 remains experimental until the cache is bounded/versioned and joined wall wins. Evidence: `tables/arrow-lake/ivfpq-b3.md`. |
| Remaining work after v0.2 ABI | Kernel quality + scale, not more algorithm C ABI names | 2026-08-29 status: symbols cover the plan’s algorithm list; the additive IVF-PQ telemetry ABI brings the export count to 111. AUTO dense search on Arrow Lake is CPU oneMKL; CAGRA/IVF do not beat matched hnswlib/FAISS gates; Lunar Lake is unmeasured. The current-code SIFT1M CAGRA point passes recall closeness at 0.9036 versus 0.8915, but hnswlib was 6.02× faster in the load-contaminated diagnostic run and the ≥0.95 target is open. Persistent IVF-PQ storage, synchronous fixed buckets, the forced bounded affine transform, shortlist-only ID resolution, fused FORCE_GPU scan/select, and complete-call stage telemetry are complete. Negative NPU and GPU measurements keep AUTO on the measured CPU route. Next are certified Q/T factorization, byte-budgeted batching, persistent GPU workspaces, and batched/fused refinement, then corrected/accelerated RaBitQ (B3–B4), while B2 throughput and full B1 curves/energy remain open. Journal: `.claude/backlog.md`. |
...
- **IVF-PQ accelerated paths (correctness guarded; persistent CSR, synchronous NPU batching, bounded affine transform, shortlist-only IDs, fused iGPU scan/select, and complete-call telemetry complete):** raw NPU ADC requires every LUT's conservative bound below 65,504; FORCE_NPU may apply the bounded affine transform and publishes atomically. FORCE_GPU scans persistent codes, filters, and emits only validated shortlist positions. AUTO retains the measured CPU route. At bounded `nprobe=8`, telemetry attributes 73.6% of CPU wall to LUT construction and 85.5% of GPU wall to fused ADC plus final per-query TopK; the scaled NPU primitive remains 73–106× slower than CPU in its diagnostic shapes. No AUTO, competitor, or energy win is claimed. Certified exact Q/T factorization, a workspace-byte block budget, persistent GPU workspaces, batched/fused refinement, and bounded/versioned NPU request caching remain. Evidence: `tables/arrow-lake/ivfpq-b3.md`.
