...
- **CAGRA SYCL B2 checkpoint:** work-group/query, SLM candidates, bounded hash, cooperative distance, and forced-policy honesty are implemented. On the final 2K smoke, recall@10 was 0.990625 at `itopk=32/search_width=1` versus hnswlib 0.99375, but QPS was about 7.4K versus 71.7K. Hash seeds, serial selection/sort, per-search copies, B5, and SIFT1M quality/performance remain open. Evidence: `tables/arrow-lake/bench-recall-qps.md`.
- **NN-Descent SYCL B5 checkpoint:** n>4096 L2-family/IP/cosine construction now uses a bounded sampled local join on iGPU with deterministic double buffering and SLM Bloom-assisted de-duplication. Isolated-process medians were 107.1 ms at 4,097 rows (0.7266 sampled exact overlap) and 832.6 ms at 16,384 rows (0.2832); first-use/outlier ranges are retained in `tables/arrow-lake/nndescent-b5.md`. These are scale/correctness regressions, not SIFT1M closure. The benchmark build-policy separation and full downstream gate remain open.
