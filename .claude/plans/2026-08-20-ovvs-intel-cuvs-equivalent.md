...
| IVF-PQ ADC batching | bounded ordered query/list descriptors; fixed NPU buckets `{128,256,512,1024,2048}`; fused iGPU scan/select | Equal NPU buckets pack into adaptive power-of-two-capacity `Gather(axis=1,batch_dims=1)` requests with request-owned tensors; only real rows are published. FORCE_GPU scans persistent uint8 codes, applies filters, and emits only an exact per-query shortlist without a dense score buffer. Search preserves query/probe/list-row ordering and whole-call publication. Clean bounded `nprobe=8` throughput was 2,336 QPS GPU versus 31,885 QPS CPU, so AUTO remains unpromoted. Replace the raw-row block cap with a workspace-byte budget after stage telemetry; exact query/list LUT factorization and persistent workspaces are next. NPU depth 1/2/4 remains experimental. Evidence: `tables/arrow-lake/ivfpq-b3.md`. |
| Remaining work after v0.2 ABI | Kernel quality + scale, not more C ABI names | 2026-08-29 status: symbols cover the plan’s algorithm list; AUTO dense search on Arrow Lake is CPU oneMKL; CAGRA/IVF do not beat matched hnswlib/FAISS gates; Lunar Lake is unmeasured. The current-code SIFT1M CAGRA point passes recall closeness at 0.9036 versus 0.8915, but hnswlib was 6.02× faster in the load-contaminated diagnostic run and the ≥0.95 target is open. Persistent IVF-PQ storage, synchronous fixed buckets, the forced bounded affine transform, shortlist-only ID resolution, and fused FORCE_GPU scan/select are complete. Negative NPU and GPU measurements keep AUTO on the measured CPU route. Next are complete-call telemetry, query/list LUT factorization, byte-budgeted batching, persistent GPU workspaces, and batched refinement, then corrected/accelerated RaBitQ (B3–B4), while B2 throughput and full B1 curves/energy remain open. Journal: `.claude/backlog.md`. |
...
- **IVF-PQ accelerated paths (correctness guarded; persistent CSR, synchronous NPU batching, bounded affine transform, shortlist-only IDs, and fused iGPU scan/select complete):** raw NPU ADC requires every LUT's conservative bound below 65,504; FORCE_NPU may apply the bounded affine transform and publishes atomically. FORCE_GPU scans persistent codes, filters, and emits only validated shortlist positions. AUTO retains the measured CPU route. Clean bounded `nprobe=8` throughput was 2,336 QPS GPU versus 31,885 QPS CPU, while the scaled NPU primitive remained 73–106× slower than CPU in its diagnostic shapes. No AUTO, competitor, or energy win is claimed. Complete-call telemetry, exact query/list LUT factorization, a workspace-byte block budget, persistent GPU workspaces, batched refinement, and bounded/versioned NPU request caching remain. Evidence: `tables/arrow-lake/ivfpq-b3.md`.
