...
| IVF-PQ ADC batching | bounded ordered query/list descriptors; fixed buckets `{128,256,512,1024,2048}`; synchronous depth-one NPU batch | Equal buckets pack into adaptive power-of-two-capacity `Gather(axis=1,batch_dims=1)` requests with request-owned tensors; only real rows are published. Search blocks cap raw candidates at index `n`, preserve candidate order, reuse refinement storage, stage the complete result, and resolve only `krefine` offsets to IDs for unfiltered search. A clean bounded A/B improved `nprobe=8` AUTO/CPU QPS by 1.10×/1.12× with unchanged recall; CPU remains 1.51× faster. Adaptive-shape depth 1/2/4 remains an experiment. Evidence: `tables/arrow-lake/ivfpq-b3.md`. |
| Remaining work after v0.2 ABI | Kernel quality + scale, not more C ABI names | 2026-08-28 status: symbols cover the plan’s algorithm list; AUTO dense search on Arrow Lake is CPU oneMKL; CAGRA/IVF do not beat matched hnswlib/FAISS gates; Lunar Lake is unmeasured. The current-code SIFT1M CAGRA point passes recall closeness at 0.9036 versus 0.8915, but hnswlib was 6.02× faster in the load-contaminated diagnostic run and the ≥0.95 target is open. Persistent IVF-PQ storage, synchronous fixed buckets, the forced bounded affine transform, and shortlist-only ID resolution are complete. The transform's negative primitive bakeoff keeps AUTO on CPU. Next are bounded/versioned caches, stage telemetry, and iGPU fused scan/select, then corrected/accelerated RaBitQ (B3–B4), while B2 throughput and full B1 curves/energy remain open. Journal: `.claude/backlog.md`. |
