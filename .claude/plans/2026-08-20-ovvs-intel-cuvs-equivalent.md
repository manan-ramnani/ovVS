...
| CAGRA walk | `prim_graph_walk`: fused SYCL when built; adaptive host fallback otherwise | **B2 full quality gate failed:** one work-group/query, cooperative distance, SLM candidate/expanded state, traversal-budget global visited hash, chunked hash allocation, and strict device USM are implemented. The pre-change SIFT1M gate recorded CAGRA recall@10 0.4057 versus hnswlib 0.8903 at the nominal public M=16/effort=32 point. The fixed walk scores at most 3,584 unique rows/query (0.3584% of SIFT1M). Query-content hashing now gives batch/order-invariant CPU/GPU seeds, and three current-code noncanonical SIFT100K preflights reached CAGRA 0.9718 versus hnswlib median 0.9510 with all 192 calls per run attributed to GPU and `192/192/0/0` transfer deltas. hnswlib was still 5.64× faster by median QPS, and the prefix does not replace the full gate. Graph-aware landmarks, explicit effort controls, and quality closure remain before QPS tuning. Forced GPU rejection never host-fallbacks; FORCE_NPU graph walk is unavailable. |
| NN-Descent build | exact primitive path through n=4096; bounded sampled iGPU NEW/OLD join above it | **B5 partial; downstream full gate not rerun:** deterministic cached-distance NEW/OLD tagging, bounded forward/reverse samples, NEW–NEW and NEW–OLD reciprocal proposals, target-owned merges, and exact changed/pending-NEW convergence accounting execute on SYCL. Bounded sampled exact overlap is 0.9727 at n=4,097/K=8 and 0.8750 at n=16,384/K=16, but both fixtures exhaust their iteration budgets. Three current-code SIFT100K AUTO constructions took 8.672–8.800 s (8.686 s median), used 398.2–398.5 MiB isolated peak process RSS, and supported 0.9718 downstream recall. The latest SIFT1M gate used the previous initializer and remains a 0.4057 measured failure, not a current-code result. Estimated SIFT1M/K64 owned device allocation is 1.116 GiB with a device-accessible dataset, while worst-case work remains large. Host CAGRA optimization is still CPU, so full FORCE_GPU build rejects until the T13.3 SYCL port. Evidence: `tables/arrow-lake/nndescent-b5.md`. |
| CAGRA graph optimization | detour-rank prune plus capped reverse-edge interleave | The SIFT1M failure and code audit show that nearest-only truncation can discard routing edges even when local kNN overlap looks acceptable. Record initializer and post-prune recall/connectivity separately; first re-test final/intermediate degree 32/64 while retaining 16/32 as the low-memory point. |
...
| Remaining work after v0.2 ABI | Kernel quality + scale, not more C ABI names | 2026-08-28 assessment: symbols cover the plan’s algorithm list; AUTO dense search on Arrow Lake is CPU oneMKL; CAGRA/IVF do not beat hnswlib/FAISS; Lunar Lake is unmeasured. The strict SIFT1M CAGRA gate exists and fails by 0.4846 recall, so the current path is B2/B5 diagnosis → real NN-Descent + CAGRA graph optimization → repeat the gate → IVF-PQ/RaBitQ search rewrite (B3–B4). Full B1 curves/energy and the 100K×768 corpus remain open. Journal: `.claude/backlog.md`. |
...
- **FAISS/hnswlib pip:** `faiss-cpu` 1.15.0 and `hnswlib` 0.8.0 import. SIFT **n=2000** artifacts remain smoke only. The first strict SIFT1M pair is valid negative quality evidence: CAGRA 0.4057 recall@10 versus hnswlib 0.8903. Three current-code SIFT100K prefix preflights measured CAGRA 0.9718 versus hnswlib median 0.9510, but hnswlib remains 5.64× faster by median QPS and the prefix is neither the SIFT1M verdict nor the full B1 curve/energy report. AUTO/CPU brute beats FAISS only on the bounded slice; the legacy NPU brute result is invalid due FP16-range saturation and now fails closed; IVF paths remain behind FAISS.
- **CAGRA SYCL B2 checkpoint:** work-group/query, SLM candidates, bounded hash, cooperative distance, and forced-policy honesty are implemented. The 2K smoke masked scale because 512 seeds cover 25.6% of that corpus and the theoretical 3,584-row visit ceiling exceeds n. On SIFT1M that ceiling is 0.3584%; recall@10 fell to 0.4057 versus hnswlib 0.8903. Resource-local counters prove direct GPU-accessible index pointers and zero explicit dataset/graph uploads at n=4,200, across the 2K smoke, and in all 192 SIFT1M calls; they do not measure driver-managed page migration. Query-content hashing fixes batch/order dependence with exact CPU/GPU parity regressions; graph-aware entry points, effort scaling, and leader-serial selection/sort remain. Evidence: `tables/arrow-lake/bench-recall-qps.md`, `tables/arrow-lake/cagra-transfer-b2.md`.
- **NN-Descent SYCL B5 checkpoint:** n>4096 L2-family/IP/cosine construction now uses cached-distance NEW/OLD tags, bounded sampled forward/reverse lists, NEW–NEW and NEW–OLD reciprocal proposals, deterministic target-owned merges, and exact changed/pending-NEW accounting. Five-process medians were 1,022.4 ms at 4,097 rows (0.9727 sampled exact overlap) and 1,931.4 ms at 16,384 rows (0.8750); both fixtures exhausted their 6/4-iteration budgets. Three current-code SIFT100K AUTO builds took 8.672–8.800 s within 398.2–398.5 MiB isolated peak process RSS and produced 0.9718 downstream recall. The existing SIFT1M failure measured the old initializer and nearest-only host prune; it predates this rewrite and the detour-rank/reverse optimizer. No current-code SIFT1M practicality, recall, or performance improvement is claimed. Evidence: `tables/arrow-lake/nndescent-b5.md`.
