...
| CAGRA walk | `prim_graph_walk`: fused SYCL when built; adaptive host fallback otherwise | **B2 checkpoint:** one work-group/query, cooperative distance, SLM candidate/expanded state, traversal-budget global visited hash, chunked hash allocation, strict device USM. `cagra_sycl_walk_n_over_4096` uses AUTO build then covers FORCE_GPU search at n=4200. Resource-local completed-walk-call counters show `2/2/0/0` walk calls/direct calls/upload calls/upload bytes there and zero uploads across the 2K smoke; SIFT1M transfer evidence, hash seeds, leader-serial selection/sort, and the full gate remain open. Forced GPU rejection never host-fallbacks; FORCE_NPU graph walk is unavailable. |
| NN-Descent build | exact primitive path through n=4096; sampled iGPU local join above it | **B5 checkpoint:** deterministic unique initialization, work-group/vertex, cooperative distance, double-buffered graph, bounded SLM candidates/Bloom, and `O(n×degree)` transient graph memory. Arrow Lake isolated regressions cover n=4097 and n=16384. Standalone FORCE_GPU is honest; AUTO CAGRA can use the initializer but finishes with CPU prune, so full FORCE_GPU CAGRA build rejects until T13.3. SIFT1M downstream quality and convergence/new-edge scheduling remain open. Evidence: `tables/arrow-lake/nndescent-b5.md`. |
...
- **FAISS/hnswlib pip:** `faiss-cpu` 1.15.0 and `hnswlib` 0.8.0 import. SIFT **n=2000** smoke artifacts are not SIFT1M. AUTO/CPU brute beats FAISS on that slice; the legacy NPU brute result is invalid due FP16-range saturation and now fails closed; IVF paths and CAGRA still lose QPS to FAISS/hnswlib. Full-corpus reports remain backlog B1.
- **CAGRA SYCL B2 checkpoint:** work-group/query, SLM candidates, bounded hash, cooperative distance, and forced-policy honesty are implemented. On the final 2K smoke, recall@10 was 0.990625 at `itopk=32/search_width=1` versus hnswlib 0.99375, but QPS was about 7.4K versus 71.7K. Resource-local counters prove direct GPU-accessible index pointers and zero explicit dataset/graph uploads at n=4,200 and across the 2K smoke; they do not measure driver-managed shared-memory page migration. SIFT1M transfer evidence, hash seeds, serial selection/sort, and SIFT1M quality/performance remain open. Evidence: `tables/arrow-lake/bench-recall-qps.md`, `tables/arrow-lake/cagra-transfer-b2.md`.
- **NN-Descent SYCL B5 checkpoint:** n>4096 L2-family/IP/cosine construction now uses a bounded sampled local join on iGPU with deterministic double buffering and SLM Bloom-assisted de-duplication. Isolated-process medians were 107.1 ms at 4,097 rows (0.7266 sampled exact overlap) and 832.6 ms at 16,384 rows (0.2832); first-use/outlier ranges are retained in `tables/arrow-lake/nndescent-b5.md`. These are scale/correctness regressions, not SIFT1M closure. Benchmark construction now defaults to AUTO independently of the search lanes and records build status/timing/final-primitive/fallback evidence; the full downstream gate remains open.
