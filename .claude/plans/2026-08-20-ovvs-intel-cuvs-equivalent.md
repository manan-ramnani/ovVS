...
- **FAISS/hnswlib pip:** `faiss-cpu` 1.15.0 and `hnswlib` 0.8.0 import. SIFT **n=2000** artifacts remain smoke only. Three clean strict SIFT1M low-effort pairs pass recall closeness: CAGRA 0.9036 versus hnswlib median about 0.891. The promoted cooperative-pick checkpoint raises current CAGRA `64/2` to 3,243.5 QPS at 0.9609 recall versus hnswlib 6,784.5 QPS at 0.9593; the 2.09× deficit remains negative product evidence. Cooperative minima puts ovVS construction ahead of the packaged stock hnswlib cohort, but the installed Windows extension is SSE-only and uses one search thread for batches 1 and 32. A pinned `/arch:AVX2` build plus batch 256/1024 points is required without weakening M/ef settings. AUTO/CPU brute beats FAISS only on the bounded slice; the legacy NPU brute result is invalid due FP16-range saturation and now fails closed; IVF paths remain behind FAISS. Other policies/algorithms, matched FAISS curves, a same-OS ScaNN control, and the real 100K×768 corpus keep B1 open.
