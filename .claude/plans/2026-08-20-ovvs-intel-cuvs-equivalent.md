...
| CAGRA walk | `prim_graph_walk`: fused SYCL when built; adaptive host fallback otherwise | **B2 ≥0.95 recall point observed; throughput open:** one work-group/query, cooperative distance, SLM candidate/expanded state, traversal-budget global visited hash, chunked hash allocation, and strict device USM are implemented. The exact cached-worst change is promoted from three old-source and three candidate SIFT1M processes: QPS improved 81.9–156.5%, p50 fell 44.8–61.8%, p99 fell 43.7–61.8%, recall remained exact, and old/new DLL output bytes matched on a retained serialized graph. Current `64/2` reaches 0.9609 recall and 2,549.9 QPS versus hnswlib 0.9589 and 6,746.8 QPS, so the product gap remains 2.65× there and 4.19× near 0.989 recall. Continue exact prefix-only final selection, stable one-pass pick, and canonical frontier tiling before separately gating cooperative selection and an owner/helper multi-work-group small-batch route. Forced GPU rejection never host-fallbacks; FORCE_NPU graph walk is unavailable. Evidence: `tables/arrow-lake/cagra-cached-worst-v1.md`. |
...
- **FAISS/hnswlib pip:** `faiss-cpu` 1.15.0 and `hnswlib` 0.8.0 import. SIFT **n=2000** artifacts remain smoke only. Three clean strict SIFT1M low-effort pairs pass recall closeness: CAGRA 0.9036 versus hnswlib median 0.8911. The repeated cached-worst A/B checkpoint raises current CAGRA `64/2` to 2,549.9 QPS at 0.9609 recall versus hnswlib 6,746.8 QPS at 0.9589; the 2.65× deficit remains negative product evidence. AUTO/CPU brute beats FAISS only on the bounded slice; the legacy NPU brute result is invalid due FP16-range saturation and now fails closed; IVF paths remain behind FAISS. Other policies/algorithms, matched FAISS curves, and the real 100K×768 corpus keep B1 open.
