...
| CAGRA walk | `prim_graph_walk`: fused SYCL when built; adaptive host fallback otherwise | **B2 ≥0.95 recall point observed; throughput open:** one work-group/query, cooperative distance, SLM candidate/expanded state, traversal-budget global visited hash, chunked hash allocation, and strict device USM are implemented. Cached-worst tracking is promoted. Prefix-only publication sorting plus canonical 64-ID frontier tiling passed a second exact six-process gate: QPS improved 8.0–10.9%, p50 fell 7.3–9.7%, p99 fell 7.2–11.0%, recall remained exact, and old/new DLL output bytes matched on a retained serialized graph. Current `64/2` reaches 0.9609 recall and 2,762.9 QPS versus hnswlib 0.9602 and 6,678.4 QPS, so the product gap remains 2.42× there and 3.84× near 0.989 recall. The exact stable one-pass pick was rejected after regressing three effort points. Continue with cooperative integer/min-location selection and a separately gated owner/helper multi-work-group small-batch route. Forced GPU rejection never host-fallbacks; FORCE_NPU graph walk is unavailable. Evidence: `tables/arrow-lake/cagra-frontier-v1.md`. |
...
- **FAISS/hnswlib pip:** `faiss-cpu` 1.15.0 and `hnswlib` 0.8.0 import. SIFT **n=2000** artifacts remain smoke only. Three clean strict SIFT1M low-effort pairs pass recall closeness: CAGRA 0.9036 versus hnswlib median 0.8911. The promoted frontier checkpoint raises current CAGRA `64/2` to 2,762.9 QPS at 0.9609 recall versus hnswlib 6,678.4 QPS at 0.9602; the 2.42× deficit remains negative product evidence. AUTO/CPU brute beats FAISS only on the bounded slice; the legacy NPU brute result is invalid due FP16-range saturation and now fails closed; IVF paths remain behind FAISS. Other policies/algorithms, matched FAISS curves, and the real 100K×768 corpus keep B1 open.
