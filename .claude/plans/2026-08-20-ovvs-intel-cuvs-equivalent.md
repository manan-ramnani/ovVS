...
| NN-Descent build | exact primitive path through n=4096; sampled iGPU outgoing join above it | **B5 downstream quality failed:** deterministic initialization, work-group/vertex, cooperative distance, double buffering, bounded SLM candidates/Bloom, and `O(n×degree)` transient graph memory execute at scale, but the first AUTO SIFT1M CAGRA build took an observed 830.0 s and fed the failed 0.4057 recall gate. The current refinement lacks reverse NEW/OLD joins, bidirectional updates, and convergence. Host CAGRA optimization now implements detour-rank pruning, a capped reverse graph, and deterministic interleave; it is unmeasured at scale and remains CPU, so full FORCE_GPU build rejects until the T13.3 SYCL port. Evidence: `tables/arrow-lake/nndescent-b5.md`. |
...
- **NN-Descent SYCL B5 checkpoint:** n>4096 L2-family/IP/cosine construction uses a bounded outgoing sampled join on iGPU with deterministic double buffering and SLM Bloom-assisted de-duplication. Isolated-process medians were 107.1 ms at 4,097 rows (0.7266 sampled exact overlap) and 832.6 ms at 16,384 rows (0.2832); first-use/outlier ranges are retained. The SIFT1M downstream gate now fails, consistent with missing reverse NEW/OLD joins, bidirectional proposals, and convergence plus a nearest-only host prune. The observed 830.0 s build time was under unrelated iGPU load and is diagnostic. Evidence: `tables/arrow-lake/nndescent-b5.md`.
