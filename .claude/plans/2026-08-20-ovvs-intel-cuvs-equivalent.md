...
| IVF-PQ ADC batching | bounded ordered query/list descriptors; fixed buckets `{128,256,512,1024,2048}`; synchronous depth-one NPU batch | Equal buckets pack into adaptive power-of-two-capacity `Gather(axis=1,batch_dims=1)` requests with request-owned tensors; only real rows are published. Search blocks cap raw candidates at index `n`, preserve candidate order, reuse refinement storage, and stage the complete result. A six-chunk fixture executes as three requests and a split/regroup fixture covers bucket 2,048 live. The earlier latency probe reported one optimal request; adaptive-shape depth 1/2/4 remains a measured experiment. Clean-code SIFT-prefix evidence has AUTO 1.40× slower than CPU at `nprobe=8`; forced NPU is range-unavailable. Evidence: `tables/arrow-lake/ivfpq-b3.md`. |
| Remaining work after v0.2 ABI | Kernel quality + scale, not more C ABI names | 2026-08-28 status: symbols cover the plan’s algorithm list; AUTO dense search on Arrow Lake is CPU oneMKL; CAGRA/IVF do not beat matched hnswlib/FAISS gates; Lunar Lake is unmeasured. The current-code SIFT1M CAGRA point passes recall closeness at 0.9036 versus 0.8915, but hnswlib was 6.02× faster in the load-contaminated diagnostic run and the ≥0.95 target is open. Persistent IVF-PQ storage, synchronous fixed buckets, and the forced bounded affine transform are complete. The transform's negative primitive bakeoff keeps AUTO on CPU. Next are bounded/versioned caches, stage telemetry, and iGPU fused scan/select, then corrected/accelerated RaBitQ (B3–B4), while B2 throughput and full B1 curves/energy remain open. Journal: `.claude/backlog.md`. |
...
- **IVF-PQ NPU ADC (correctness guarded, persistent CSR, synchronous batching, and forced affine transform complete):** raw `prim_pq_adc_batch` execution requires every LUT's conservative bound below 65,504. FORCE_NPU may subtract subspace minima, scale the remaining span to 60,000 at scale ≥0.5, restore the bias after validation, and publish atomically. AUTO retains the unsafe CPU fallback. Build/load/extend derive validated list-major codes; bounded search preserves order and complete-result publication. Equal `{128,256,512,1024,2048}` buckets pack into adaptive depth-one Gather requests. A scale-0.588 fixture retained 32/32 top candidates with 27.25 maximum score error, while a wider transform rejected. The scaled primitive was 73–106× slower than CPU in current diagnostic shapes, so no AUTO, competitor, or energy win is claimed. Bounded/versioned request caching, stage telemetry, depth experiments, and iGPU fused scan/select remain. Evidence: `tables/arrow-lake/ivfpq-b3.md`.
