...
| Remaining work after v0.2 ABI | Kernel quality + scale, not more C ABI names | 2026-08-28 status: symbols cover the plan’s algorithm list; AUTO dense search on Arrow Lake is CPU oneMKL; CAGRA/IVF do not beat hnswlib/FAISS; Lunar Lake is unmeasured. The current-code SIFT1M CAGRA point passes recall closeness at 0.9036 versus 0.8915, but hnswlib was 6.02× faster in the load-contaminated diagnostic run and the ≥0.95 target is open. Persistent IVF-PQ list-major storage is complete; the next implementation path is batched work descriptors/fixed ADC buckets and request pools, then RaBitQ (B3–B4), while B2 throughput and full B1 curves/energy remain open. Journal: `.claude/backlog.md`. |
...
- **IVF-PQ NPU ADC (correctness guarded, persistent CSR complete, batching open):** `prim_pq_adc` Gather+ReduceSum tiles run ActShave+DPU only when the conservative LUT accumulation bound is below 65,504 and every output tile validates; caller output is published atomically after the full request batch. Unsafe AUTO falls back to CPU; FORCE_NPU fails unavailable. Build/load/extend derive validated list offsets, IDs, and contiguous codes, so unfiltered search no longer repacks codes. Query residual/LUT construction and ADC dispatch remain per query/list. Three bounded current-code runs kept exact AUTO/CPU recall parity but AUTO was 9.14× slower than CPU at `nprobe=8`; fixed buckets/request pools are still infer-only directions, not end-to-end proof. Evidence: `tables/arrow-lake/ivfpq-b3.md`.
