...
| Remaining work after v0.2 ABI | Kernel quality + scale, not more algorithm C ABI names | 2026-08-29 status: symbols cover the plan’s algorithm list; the additive IVF-PQ and CAGRA telemetry ABIs bring the export count to 112. Three clean SIFT1M CAGRA gates pass recall closeness at 0.9036 versus hnswlib median 0.8911, but CAGRA delivers 30.35% of hnswlib search QPS, builds 1.302× slower, and remains below the ≥0.95 target. HNSW export interoperability is valid but loses build and nearest comparable-recall search at SIFT100K. Persistent IVF-PQ storage, buckets, numeric guards, shortlist-only IDs, fused FORCE_GPU scan/select, and both telemetry contracts are complete; Q/T and Arrow Lake NPU escape routes are negative within their documented scopes. Next are measured GPU NN-Descent synchronization/readback reduction, a SIMD16 CAGRA search portfolio, hierarchical/packed iGPU PQ, and size-gated CPU+iGPU batching while full B1 curves/energy remain open. Journal: `.claude/backlog.md`. |
...
- **FAISS/hnswlib pip:** `faiss-cpu` 1.15.0 and `hnswlib` 0.8.0 import. SIFT **n=2000** artifacts remain smoke only. Three clean strict SIFT1M pairs pass recall closeness: CAGRA 0.9036 versus hnswlib median 0.8911. CAGRA remains below the ≥0.95 target and delivers 30.35% of hnswlib median search throughput; the matched point is not the full B1 curve/energy report. AUTO/CPU brute beats FAISS only on the bounded slice; the legacy NPU brute result is invalid due FP16-range saturation and now fails closed; IVF paths remain behind FAISS.
...
- **CAGRA SYCL B2 checkpoint:** work-group/query, SLM candidates, bounded hash, cooperative distance, and forced-policy honesty are implemented. Three clean full SIFT1M gates pass recall closeness at 0.9036 versus hnswlib median 0.8911. Resource-local counters prove direct GPU-accessible index pointers, all 192 device attributions per process, and zero explicit dataset/graph uploads; they do not measure driver-managed page migration. Query-content hashing fixes batch/order dependence with exact CPU/GPU parity regressions. The ≥0.95 target, graph-aware entry points, effort scaling, and leader-serial selection/sort remain; median CAGRA search throughput is 30.35% of hnswlib. Evidence: `tables/arrow-lake/bench-recall-qps.md`, `tables/arrow-lake/cagra-transfer-b2.md`, `tables/arrow-lake/cagra-build-v1.md`.
- **NN-Descent SYCL B5 checkpoint:** n>4096 L2-family/IP/cosine construction uses cached-distance NEW/OLD tags, bounded sampled forward/reverse lists, NEW–NEW and NEW–OLD reciprocal proposals, deterministic target-owned merges, and exact changed/pending-NEW accounting. Three clean SIFT1M AUTO builds have a 100.093 s median and 2,343.2 MiB median isolated peak RSS; the GPU initializer contributes 90.444 s. All runs exhaust six iterations, while downstream CAGRA recall 0.9036 passes matched hnswlib closeness. This does not close T12.5: standalone convergence, the IVF-PQ-init comparison, K64 scale, and the T13.3 GPU prune remain open. Evidence: `tables/arrow-lake/cagra-build-v1.md`, `tables/arrow-lake/nndescent-b5.md`.
