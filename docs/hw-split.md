...
| PQ ADC | bounded fixed-bucket batch fallback | SHAVE C | **Batched Gather+ReduceSum when range-safe** | — | NPU only for whole-batch safe bounds; CPU fallback otherwise |
...
- A current-stack direction probe reported one optimal request in latency mode and four in throughput mode. Fixed-shape ADC batching now packs compatible list chunks into one or more capacity-bounded synchronous requests while retaining a single cached request/mutex per shape. A reusable `start_async` pool at depths 2/4 remains a separate primitive concurrency experiment, not a HETERO pipeline claim. Measure queueing, overlap, memory, energy, and tail latency before changing the depth-one production default.
...
Complete-call V1 telemetry now attributes successful search wall and explicit GPU work. At the bounded `nprobe=8` point, FORCE_CPU reached 32,074 QPS and spent a median 73.6% of native wall constructing LUTs. FORCE_GPU reached 2,284 QPS and spent 45.5% in fused ADC plus 40.0% in final per-query TopK, while issuing 99 allocations, 120 logical kernel submissions, and 230 waits per call. AUTO remains on CPU. The next CPU experiment is exact residual-PQ Q/T factorization with a conservative cutoff-gap certificate and direct-LUT fallback. The next GPU experiment must batch/fuse exact refinement and then reuse bounded workspaces; allocation reuse by itself cannot remove the measured synchronized primitive tax. Replace the raw-row cap with a workspace-byte budget, bound/version the NPU request cache before depth experiments, and require repeated matched SIFT1M evidence before promotion.
...
- IVF-PQ ADC → CPU/NPU under the existing measured rules; the fused iGPU scan/select is FORCE_GPU-only until its end-to-end table wins
