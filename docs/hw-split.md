...
Current B3 evidence is in `tables/arrow-lake/ivfpq-b3.md`. The fail-closed guard restored bounded SIFT-prefix AUTO/FORCE_CPU recall parity. Persistent list-major CSR removes per-list code repacking and temporary list-ID scratch on unfiltered search; final candidate-ID aggregation remains. Lifecycle tests keep `IPQ1` v1 byte-compatible and extend transactional. Three current-code smokes still left AUTO 9.14× slower than FORCE_CPU at `nprobe=8`. Batched descriptors, fixed buckets, request pools, and iGPU scan/select remain open; infer-only probes are not end-to-end wins.
