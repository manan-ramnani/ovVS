...
Current B3 evidence is in `tables/arrow-lake/ivfpq-b3.md`. The fail-closed guard preserves bounded SIFT-prefix AUTO/FORCE_CPU recall parity. Persistent list-major CSR removes per-list code repacking, and bounded query/list descriptors now feed synchronous fixed-bucket NPU graphs while preserving candidate order and whole-search publication. Lifecycle tests keep `IPQ1` v1 byte-compatible and extend transactional. Three pre-commit development smokes measured AUTO/FORCE_CPU at 23,491.4/32,533.6 median QPS at `nprobe=8`, changing the checkpoint ratio from 9.14× to 1.38× without changing 0.946875 recall. This is a diagnostic SIFT-prefix result, not an isolated batching or matched FAISS win: the artifacts are dirty, forced NPU stayed unavailable on the wide-range data, raw FAISS had lower recall, and energy was disabled. Per-LUT scaling, bounded/versioned request caches, request-depth bakeoffs, iGPU scan/select, and repeated SIFT1M evidence remain open.
