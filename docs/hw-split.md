...
Current B3 evidence is in `tables/arrow-lake/ivfpq-b3.md`. The fail-closed guard preserves bounded SIFT-prefix AUTO/FORCE_CPU recall parity. Persistent list-major CSR removes per-list code repacking, and bounded query/list descriptors feed synchronous fixed-bucket NPU graphs while preserving candidate order and whole-search publication. Unfiltered search also avoids the dense 8-byte candidate-ID payload and resolves only `krefine` offsets; a clean bounded A/B improved `nprobe=8` AUTO/CPU QPS by 1.10×/1.12× with unchanged recall, while CPU remained 1.51× faster. Lifecycle tests keep `IPQ1` v1 byte-compatible and extend transactional. The forced affine transform removes candidate-invariant LUT offsets, accepts scales down to 0.5, and restores each task's bias after validated inference. A deterministic scale-0.588 fixture retained 32/32 top candidates with 27.25 maximum score error; a wider scale-0.029 fixture failed atomically. This is correctness evidence only: three-process hot primitive medians left NPU about 106× slower than CPU at 131,072 candidates and 73.3× slower at 524,288. AUTO remains CPU for unsafe LUTs. Bounded/versioned request caching, stage telemetry, iGPU scan/select, and repeated matched SIFT1M evidence remain open.
