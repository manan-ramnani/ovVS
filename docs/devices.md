...
Bounded sampled exact overlap is 0.9727 at n=4,097/K=8 and 0.8750 at n=16,384/K=16, but both cases exhaust their small iteration budgets without convergence. The first SIFT1M gate predates this rewrite, the query-seed fix, and the host graph optimizer; its 0.4057 recall remains the latest measured failure, not a current-code result. AUTO CAGRA build applies deterministic host detour-rank pruning with a capped reverse graph and forward/reverse interleave and reports CPU for that final stage. Full FORCE_GPU CAGRA build therefore returns `DEVICE_UNAVAILABLE` until T13.3 moves optimization to GPU. SIFT1M practicality, tuning, peak process memory, and downstream recall remain open.
