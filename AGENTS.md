...
2. The exact T13.3 GPU optimizer and cooperative proposal minima are promoted. Three AUTO SIFT1M builds reduce complete wall from the current 91.978-second product median to 61.000 seconds (-33.68%) and initializer wall from 90.883 to 60.164 seconds (-33.80%), with exact 100K/1M graph bytes, unchanged 0.9036 recall and lifecycle counters, +0.04% search QPS, +0.03% p99, and +0.14% peak RSS. Full FORCE_GPU NN-Descent CAGRA construction completes for supported shapes. The same cohort builds 20.87% faster than the packaged stock hnswlib binary, whose float-distance arithmetic uses legacy SSE; do not claim a full-speed competitor win until the pinned `/arch:AVX2` lane is measured. Evidence: `tables/arrow-lake/nndescent-cooperative-minima-v1.md` and `tables/arrow-lake/cagra-gpu-optimize-v1.md`.
3. The remaining measured construction stage is the 60.164-second GPU NN-Descent initializer. Four synchronization designs remain parked, and bank-safe T64 plus NEW-only coordinate staging lost 2.51×/2.23× at SIFT100K. Preserve dynamic active-count launches, bulk head fills, scalar readbacks, and the promoted cooperative-minima emission order. Reprofile the current kernel before another one-variable implementation; separately gate removal of the final initializer D2H plus optimizer H2D handoff. T12.5 IVF-PQ-initializer comparison, K64 scale, and standalone convergence remain open. The stock-hnswlib export remains parked after losing build and nearest comparable-recall search. Evidence: the NN-Descent rejection reports and `tables/arrow-lake/nndescent-cooperative-minima-v1.md`.
...
6. The isolated hnswlib 0.8.0 `/arch:AVX2` builder, fail-closed semantic provenance loading, exact full-profile point-multiset gate, and targeted batch 256/1024 points are complete. Two fresh roots produced an identical wheel/module; the candidate imports and static binary contains AVX float-distance instructions absent from the packaged control. This is not active-cycle attribution or a performance conclusion. Repeated complete SIFT1M runs remain mandatory, with unchanged M/ef settings. Evidence: `tables/arrow-lake/evidence/hnswlib-avx2-build-v1/README.md`. A WSL ScaNN run may screen algorithms, but it is not a Windows product comparison unless every comparator uses the same OS. Repeat/interleave the CAGRA curve for any promotion candidate, finish the other B1 algorithms/policies and the real 100K×768 corpus. Bake off Lunar Lake only when that hardware exists, then continue v1.0/v1.1 work in plan order.
