...
2. The frozen CAGRA build telemetry ABI and six clean DLL-bound measurements are complete. At SIFT1M the median 100.09-second build spends 90.44 seconds (90.36%) in GPU NN-Descent and 9.03 seconds in host optimize/prune/merge; all runs hit the six-iteration cap. Four bounded synchronization designs are parked: fixed-`N` active counts (+8.91% complete wall), convergence reduction (+2.14%), target-owned head reset (+5.55%), and producer→copy event chaining (+8.96%). Preserve dynamic active-count launches, bulk head fills, scalar readbacks, and the original convergence path. Further initializer changes require device-side phase profiling and less algorithmic work, not another API-counter proxy. Evidence: `tables/arrow-lake/nndescent-active-count-v1.md`, `tables/arrow-lake/nndescent-convergence-reduction-v1.md`, `tables/arrow-lake/nndescent-heads-reset-v1.md`, and `tables/arrow-lake/nndescent-producer-copy-event-v1.md`.
3. Move graph optimize/prune/merge to the iGPU as the next independently measured construction change. Keep the CPU path until exact graph validation and repeated complete wall prove a winner. The stock-hnswlib export is valid and fully reachable, but its measured SIFT100K build and nearest comparable-recall search both lost; do not tune it as the active acceleration route.
