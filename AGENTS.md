...
2. The frozen CAGRA build telemetry ABI and six clean DLL-bound measurements are complete. At SIFT1M the median 100.09-second build spends 90.44 seconds (90.36%) in GPU NN-Descent and 9.03 seconds in host optimize/prune/merge; all runs hit the six-iteration cap. A device-resident active-count/fixed-`N` consumer experiment reduced D2H calls 95.42% and waits 73.19% but regressed complete build 8.91% and is parked. Preserve dynamic active-count launches; next reduce the two full convergence-array readbacks to one validated device-reduced summary, with a fresh exact parent/candidate gate. Evidence: `tables/arrow-lake/nndescent-active-count-v1.md`.
