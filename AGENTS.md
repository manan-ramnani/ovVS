...
2. The exact T13.3 GPU optimizer is promoted through intermediate degree 64. Three AUTO SIFT1M builds reduce optimizer wall from 9.034 to 0.641 seconds (-92.90%) and complete build from 100.093 to 91.987 seconds (-8.10%), with exact parent/candidate graph bytes, unchanged 0.9036 recall and initializer counters, and 3.13% lower peak RSS. Full FORCE_GPU NN-Descent CAGRA construction now completes for supported shapes. ovVS still builds 1.182× slower than unchanged hnswlib in the candidate cohort. Evidence: `tables/arrow-lake/cagra-gpu-optimize-v1.md`.
3. The remaining construction barrier is the 90.883-second GPU NN-Descent initializer. Four synchronization designs remain parked: fixed-`N` active counts (+8.91% complete wall), convergence reduction (+2.14%), target-owned head reset (+5.55%), and producer→copy event chaining (+8.96%). Preserve dynamic active-count launches, bulk head fills, scalar readbacks, and the original convergence path. Continue only from device-side phase profiling and less algorithmic work; separately gate removal of the final initializer D2H plus optimizer H2D handoff. Matching current-cohort hnswlib requires another 14.17 seconds. The stock-hnswlib export also remains parked after losing build and nearest comparable-recall search. Evidence: the four NN-Descent rejection reports and `tables/arrow-lake/cagra-gpu-optimize-v1.md`.
