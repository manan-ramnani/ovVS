...
6. In parallel, add a pinned hnswlib 0.8.0 `/arch:AVX2` comparator and batch 256/1024 throughput points without weakening M/ef settings. A WSL ScaNN run may screen algorithms, but it is not a Windows product comparison unless every comparator uses the same OS. Repeat/interleave the CAGRA curve for any promotion candidate, finish the other B1 algorithms/policies and the real 100K×768 corpus. Bake off Lunar Lake only when that hardware exists, then continue v1.0/v1.1 work in plan order.
