...
1. The clean same-index SIFT1M curve is complete. The existing graph reaches 0.9609 recall at `itopk=64/search_width=2`, but delivers only 17.51% of hnswlib QPS at comparable recall; batch-one is much worse. Exact cached-worst candidate tracking is implemented and exactness-qualified but unmeasured. Run its complete-wall gate first; retain it only on the documented p50/p99/QPS/recall and structural thresholds. Then profile deterministic SIMD16 cooperative selection, multi-work-group/query for small batches, one-work-group/query for large batches, and a bounded fallback as separate changes. Never weaken hnswlib settings.
2. The frozen CAGRA build telemetry ABI and six clean DLL-bound measurements are complete. At SIFT1M the median 100.09-second build spends 90.44 seconds (90.36%) in GPU NN-Descent and 9.03 seconds in host optimize/prune/merge; all runs hit the six-iteration cap. Reduce initializer submissions/waits/readbacks in a separate exact-graph checkpoint.
