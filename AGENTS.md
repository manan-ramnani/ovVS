...
1. The frozen, versioned, success-only CAGRA build-stage telemetry ABI is complete. Before changing construction kernels, bind the rebuilt DLL hash to the revision and run three clean SIFT100K diagnostic processes; promote to three SIFT1M processes only when every telemetry/correctness invariant passes.
2. Use those complete-build measurements to accelerate the actual CAGRA/NN-Descent and graph optimize/prune/merge bottleneck on the iGPU. The stock-hnswlib export is valid and fully reachable, but its measured SIFT100K build and nearest comparable-recall search both lost; do not tune it as the active acceleration route.
