...
1. The clean same-index SIFT1M curve is complete. The existing graph reaches 0.9609 recall at `itopk=64/search_width=2`, but delivers only 17.51% of hnswlib QPS at comparable recall; batch-one is much worse. Begin with exact cached-worst candidate tracking, then profile deterministic SIMD16 cooperative selection, multi-work-group/query for small batches, one-work-group/query for large batches, and a bounded fallback. Never weaken hnswlib settings.
