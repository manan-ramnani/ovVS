...
1. The clean same-index SIFT1M curve is complete. Cached-worst tracking, prefix-only publication sorting, and canonical 64-ID frontier tiling are promoted with repeated complete-wall and bitwise evidence. The graph reaches 0.9609 recall at `itopk=64/search_width=2` and 2,762.9 QPS, versus hnswlib 0.9602 and 6,678.4 QPS; batch one is much worse. The exact stable one-pass pick was measured and rejected. Next profile cooperative integer/min-location selection, then multi-work-group/query for small batches, one-work-group/query for large batches, and a bounded fallback as separate changes. Never weaken hnswlib settings. Evidence: `tables/arrow-lake/cagra-frontier-v1.md`.
