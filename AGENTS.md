...
2. The frozen CAGRA build telemetry ABI and six clean DLL-bound measurements are complete. At SIFT1M the median 100.09-second build spends 90.44 seconds (90.36%) in GPU NN-Descent and 9.03 seconds in host optimize/prune/merge; all runs hit the six-iteration cap. Two bounded synchronization designs are parked: fixed-`N` device-resident active counts regressed complete build 8.91%, and a convergence-only single-work-group reduction regressed it 2.14% despite 27.27% fewer D2H bytes. Preserve dynamic active-count launches and the original convergence path. Next initialize `heads[N]` once per iteration and have each unique target-owned consumer reset only the rows it touched, then run a fresh exact parent/candidate gate. Evidence: `tables/arrow-lake/nndescent-active-count-v1.md` and `tables/arrow-lake/nndescent-convergence-reduction-v1.md`.
