...
3. **Next implementation:** IVF-PQ persistent CSR, fixed ADC buckets, the bounded affine NPU lane, shortlist-only IDs, and fused FORCE_GPU scan/select are complete. The clean bounded GPU path remains 13.65× slower than CPU at `nprobe=8`, so AUTO stays off it. Continue B3 with complete-call stage telemetry, a workspace-byte block budget, exact query/list LUT factorization, persistent GPU workspaces, and batched refinement. Bound/version the NPU request cache before any depth 1/2/4 experiment. Then correct and accelerate RaBitQ (B4). Keep this software-only; do not require BIOS changes.
