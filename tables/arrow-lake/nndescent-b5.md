...
This table validates the first scale/correctness checkpoint only. It is not SIFT1M evidence, not a comparison with cuVS, and not the downstream CAGRA/hnswlib quality gate. Peak RSS was not measured. The B1 harness still builds indexes under FORCE_CPU and keeps CAGRA above 4,096 rows opt-in until a full measured build completes. AUTO CAGRA can use the GPU initializer but finishes with CPU prune; full FORCE_GPU CAGRA build remains unavailable until T13.3.

Checkpoint verification: 63/63 native tests and 6/6 CTest lanes passed; Python benchmark/fetcher suites passed 19/19 and 6/6. GPU scale lanes use explicit CTest SKIP on device absence.
