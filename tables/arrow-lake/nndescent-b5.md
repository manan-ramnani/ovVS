...
The first downstream SIFT1M gate has now run. AUTO CAGRA construction completed in an observed 830.0 s and ended with CPU prune, but CAGRA recall@10 was 0.4057 versus hnswlib 0.8903 at the nominal public M=16/effort=32 point; the graph topologies are not identical. The build time was collected under substantial unrelated iGPU load and is not an isolated performance baseline; the quality failure is valid. This confirms scale execution but rejects the measured sampled initializer/prune/search combination for product quality. The gate predates both the query-content seed fix and the host detour-rank/reverse-interleave optimizer and has not been rerun, so 0.4057 remains the latest measured failure rather than a current-code recall claim. Real NEW/OLD/reverse NN-Descent work and a search-effort diagnostic remain before attributing the full gap to one stage. Peak RSS was not measured, the new optimizer is unmeasured at scale, and full FORCE_GPU CAGRA build remains unavailable until the T13.3 SYCL port.

Checkpoint verification: 64/64 native tests and 6/6 CTest lanes passed; after the strict SIFT1M gate landed, the Python benchmark/fetcher suites pass 45/45 and 6/6. GPU scale lanes use explicit CTest SKIP on device absence.
